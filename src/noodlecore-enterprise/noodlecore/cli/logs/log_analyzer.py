"""
Logs::Log Analyzer - log_analyzer.py
...
class LogAnalyzer:
    ...
    async def _detect_error_spikes(
        ...
        baseline_error_rate = len(error_entries) / total_entries
        
        # Check for spikes in time windows
        current_time = entries[-1].timestamp
        ...
        if recent_entries:
            recent_error_rate = len(recent_errors) / len(recent_entries)
            ...
            if recent_error_rate > baseline_error_rate * self.anomaly_thresholds['error_spike_threshold']:
                anomaly = LogAnomaly(
                    ...
                    confidence=min(1.0, recent_error_rate / baseline_error_rate - 1.0),
                    details={
    ...
    async def _detect_performance_degradation(
        ...
        baseline_times = times[:len(times)//2]
        recent_times = times[len(times)//2:]
    ...
    async def _detect_repeated_failures(
        ...
        for message, group_entries in message_groups.items():
            if len(group_entries) >= self.anomaly_thresholds['repeated_failure_count']:
                ...
                time_span = (group_entries[-1].timestamp - group_entries[0].timestamp).total_seconds()
                
                if time_span <= time_window:
                    anomaly = LogAnomaly(
                        timestamp=group_entries[-1].timestamp,
                        ...
                        confidence=min(1.0, len(group_entries) / 10.0),
                        details={
    ...
    async def _detect_missing_heartbeats(
        ...
        for i in range(1, len(heartbeat_entries)):
            gap = (heartbeat_entries[i].timestamp - heartbeat_entries[i-1].timestamp).total_seconds()
            ...
            if gap > self.anomaly_thresholds['missing_heartbeat_minutes'] * 60:
                anomaly = LogAnomaly(
                    ...
                    affected_entries=[heartbeat_entries[i-1], heartbeat_entries[i]],
    ...
    async def _detect_security_anomalies(
        ...
        for pattern, anomaly_type in security_patterns:
            ...
            if len(matching_entries) >= 3:  # Threshold for security anomalies
                anomaly = LogAnomaly(
                    timestamp=matching_entries[-1].timestamp,
                    ...
                    confidence=min(1.0, len(matching_entries) / 10.0),
                    details={
                        'pattern': pattern,
