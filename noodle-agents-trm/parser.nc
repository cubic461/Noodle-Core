...
class TRMParser:
    ...
    func _parse_to_ast(self, source_code: str) -> ast.AST:
        """
    ...
    func _extract_metadata(self,
        ...
        class MetadataVisitor(ast.NodeVisitor):
            func __init__(self, metadata_dict):
                self.metadata = metadata_dict
                self.complexity_stack = [0]
            
            func visit_FunctionDef(self, node):
                self.metadata['functions'].append(node.name)
                self.generic_visit(node)
            
            func visit_AsyncFunctionDef(self, node):
                self.metadata['functions'].append(node.name)
                self.generic_visit(node)
            
            func visit_ClassDef(self, node):
                self.metadata['classes'].append(node.name)
                self.generic_visit(node)
            
            func visit_Import(self, node):
                for alias in node.names:
                    self.metadata['# imports'].append(alias.name)
                self.generic_visit(node)
            
            func visit_ImportFrom(self, node):
            ...
            func visit_If(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
            
            func visit_For(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
            
            func visit_While(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
            
            func visit_ExceptHandler(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
            
            func visit_With(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
            
            func visit_Try(self, node):
                self.complexity_stack[-1] += 1
                self.generic_visit(node)
    ...
    func _check_type_hints(self, syntax_tree: ast.AST) -> bool:
        ...
        class TypeHintVisitor(ast.NodeVisitor):
            ...
            func visit_AnnAssign(self, node):
                nonlocal has_type_hints
                has_type_hints = True
                self.generic_visit(node)
    ...
    func _check_docstrings(self, syntax_tree: ast.AST) -> bool:
    ...
    func _convert_ast_to_dict(self, syntax_tree: ast.AST) -> Dict[str, Any]:
        """
    ...
    func _generate_cache_key(self, source_code: str, module_name: str) -> str:
