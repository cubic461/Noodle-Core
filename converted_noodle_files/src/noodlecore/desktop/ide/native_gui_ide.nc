...
try
    ...
    PYGMENTS_AVAILABLE = False
        logging.error(f"Failed to import Pygments: {str(e)}")
        print("Pygments not available - syntax highlighting disabled")
...
try
    ...
    PROGRESS_MANAGER_AVAILABLE = True
        print("[OK] Progress Manager loaded successfully")
except ImportError as e
    PROGRESS_MANAGER_AVAILABLE = False
        logging.error(f"[ERROR] Progress Manager not available: {str(e)}")
        print(f"[ERROR] Progress Manager not available: {e}")
    #     # Add detailed diagnostic logging
    #     import traceback
        print(f"[ERROR] Progress Manager import traceback: {traceback.format_exc()}")
...
class ZAIProvider(AIProvider)
    #     """Z.ai API provider."""
    #     def __init__(self):
            super().__init__("Z.ai", "https://api.z.ai/v1", True)

    #     async def chat_completion(self, messages, api_key=None, model="zai-gpt-3.5-turbo"):
    #         if not api_key:
                raise Exception("Z.ai API key required")
    ...
    #         except Exception as e:
                raise Exception(f"Z.ai request failed: {str(e)}")

    #     async def get_available_models(self, api_key=None):
    #         """Get available models from Z.ai API."""
    #         if not api_key:
    #             return ["zai-gpt-3.5-turbo", "zai-gpt-4", "zai-gpt-4-turbo"]
    ...
    result = await response.json()
    #                         models = [model['id'] for model in result['data']
    #                                    if model['id'].startswith(('zai-', 'gpt-', 'text-', 'davinci'))]
                            return sorted(models)
    #                     else:
    #                         return ["zai-gpt-3.5-turbo", "zai-gpt-4", "zai-gpt-4-turbo"]
    #         except Exception as e:
                print(f"Failed to fetch Z.ai models: {e}")
    #             return ["zai-gpt-3.5-turbo", "zai-gpt-4", "zai-gpt-4-turbo"]
...
REPO_ROOT = math.subtract(CURRENT_DIR.parent.parent  # .../noodle, core)
...
try
    ...
    NoodleCommandRuntime = None  # type: ignore
        logging.error(f"NoodleCore Command Runtime not available: {str(e)}")
...
try
    ...
    NoodleCoreGitTools = None  # type: ignore
        logging.error(f"NoodleCore Git tools not available: {str(e)}")
...
try
    ...
    NoodleCoreQualityTools = None  # type: ignore
        logging.error(f"NoodleCore Quality tools not available: {str(e)}")
        print(f"NoodleCore Quality tools not available: {e}")


class IDEInitializationError(Exception)
    #     """Exception raised when IDE initialization fails."""
    #     pass
...
class NativeNoodleCoreIDE
    ...
    #         except Exception as e:
                logging.error(f"Failed to initialize main window: {str(e)}")
                raise IDEInitializationError(f"Failed to initialize IDE: {str(e)}")
    ...
    #         # Noodle runtime execution preference
    self.use_noodle_runtime_for_python = False

    #         # AI roles - integrate with role manager for centralized management
    #         try:
    #             from noodlecore.ai.role_manager import get_role_manager
    self.role_manager = get_role_manager(workspace_path=str(self.current_project_path))
    ...
    #                 # Read the actual document content for system prompt
    #                 try:
    system_prompt_content = self.role_manager.read_role_document(manager_role.id)
    #                 except Exception as e:
                        print(f"Failed to read Manager role document: {e}")
    system_prompt_content = manager_role.description

    #                 # Create compatible AI role object
    ...
    #         except Exception as e:
                logging.error(f"Failed to initialize role manager: {str(e)}")
    ...
    self.progress_manager = ProgressManager(self)
                    print("[OK] Progress Manager initialized successfully")
    #                 # Start monitoring by default
                    self.progress_manager.start_monitoring()
    #             except Exception as e:
                    logging.error(f"Failed to initialize Progress Manager: {str(e)}")
                    print(f"[ERROR] Failed to initialize Progress Manager: {e}")
    self.progress_manager = None
    #         else:
    self.progress_manager = None
                print("[WARNING] Progress Manager not available - IDE will run without progress tracking")
    ...
    self.self_improvement_integration = SelfImprovementIntegration(self)
                print("[OK] Self-Improvement Integration initialized successfully")

    #             # Auto-start Self-Improvement monitoring
    #             try:
                    self.self_improvement_integration.start_monitoring()
                    print("[OK] Self-Improvement monitoring auto-started")
    #             except Exception as e:
                    print(f"[WARN] Failed to auto-start Self-Improvement monitoring: {e}")

    #         except Exception as e:
                logging.error(f"Failed to initialize Self-Improvement Integration: {str(e)}")
                print(f"[ERROR] Failed to initialize Self-Improvement Integration: {e}")
    #             # Add detailed diagnostic logging
    #             import traceback
                print(f"[ERROR] Self-Improvement Integration traceback: {traceback.format_exc()}")
    self.self_improvement_integration = None

    #         # Initialize TRM Controller Integration if available
    #         try:
    #             from noodlecore.desktop.ide.trm_controller_integration import get_trm_controller_integration
    self.trm_controller_integration = get_trm_controller_integration(self)
                print("[OK] TRM Controller Integration initialized successfully")
    #         except Exception as e:
                logging.error(f"Failed to initialize TRM Controller Integration: {str(e)}")
                print(f"[ERROR] Failed to initialize TRM Controller Integration: {e}")
    #             # Add detailed diagnostic logging
    #             import traceback
                print(f"[ERROR] TRM Controller Integration traceback: {traceback.format_exc()}")
    ...
    self.trm_agent = TRMAgent()
                print("[OK] TRM Agent initialized successfully")
    #         except Exception as e:
                logging.error(f"Failed to initialize TRM Agent: {str(e)}")
                print(f"[ERROR] Failed to initialize TRM Agent: {e}")
    #             # Add detailed diagnostic logging
    #             import traceback
                print(f"[ERROR] TRM Agent initialization traceback: {traceback.format_exc()}")
    self.trm_agent = None
    ...
    #         # Auto-start TRM Controller if configured
    #         if hasattr(self, 'trm_controller_integration') and self.trm_controller_integration:
    #             try:
    integration_status = self.trm_controller_integration.get_integration_status()
    config = integration_status.get('config', {})

    #                 if config.get('auto_start_controller', False):
                        print("[AUTO] Auto-starting TRM Controller...")
    success = self.trm_controller_integration.start_controller()
    #                     if success:
                            print("[OK] TRM Controller auto-started successfully")
    #                     else:
                            print("[WARN] Failed to auto-start TRM Controller")
    #             except Exception as e:
                    print(f"[ERROR] Failed to auto-start TRM Controller: {e}")
    ...
    #         except Exception as e:
                logging.error(f"Failed to create UI components: {str(e)}")
                raise IDEInitializationError(f"Failed to create UI: {str(e)}")
    ...
    #         except Exception as e:
                logging.error(f"Failed to initialize content: {str(e)}")
                print(f"Warning: Failed to initialize IDE content: {e}")
    ...
    #             except Exception as e:
                    logging.error(f"Failed to initialize NoodleCore Command Runtime: {str(e)}")
    ...
    #             except Exception as e:
                    logging.error(f"Failed to initialize NoodleCore Git tools: {str(e)}")
    ...
    #             except Exception as e:
                    logging.error(f"Failed to initialize NoodleCore Quality tools: {str(e)}")
    ...
    menubar.add_cascade(label = "Self-Improvement", menu=self_improvement_menu)
            ...
            # TRM Controller menu (NEW)
    trm_menu = tk.Menu(menubar, tearoff=0, bg="#2b2b2b", fg="white")
    trm_menu.add_command(label = "TRM Controller Status", command=self.show_trm_controller_status)
    trm_menu.add_command(label = "Start TRM Controller", command=self.start_trm_controller)
    trm_menu.add_command(label = "Stop TRM Controller", command=self.stop_trm_controller)
            trm_menu.add_separator()
    trm_menu.add_command(label = "Trigger Immediate Cycle", command=self.trigger_trm_cycle)
            trm_menu.add_separator()
    trm_menu.add_command(label = "TRM Settings", command=self.show_trm_settings)
    menubar.add_cascade(label = "TRM Controller", menu=trm_menu)
    ...
    #     def _create_enhanced_layout(self) -> None:
    #         """Create enhanced IDE layout with resizable panels."""
    #         # Main container with paned windows for resizing
    main_paned = tk.PanedWindow(self.root, orient=tk.HORIZONTAL, bg="#2b2b2b")
    main_paned.pack(fill = "both", expand=True)

            # Left panel (file explorer, AI chat, self-improvement)
    left_paned = tk.PanedWindow(main_paned, orient=tk.VERTICAL, bg="#2b2b2b")

    #         # File explorer
    file_frame = tk.Frame(left_paned, bg="#2b2b2b")
    file_frame.pack(fill = "both", expand=True)

    file_label = tk.Label(file_frame, text="📁 File Explorer", bg="#2b2b2b", fg="white", font=("Arial", 10, "bold"))
    ...
    left_paned.add(file_frame, minsize = 200)

    #         # Create notebook for AI Chat and Self-Improvement tabs
    left_notebook = ttk.Notebook(left_paned)
    left_notebook.pack(fill = "both", expand=True)

    #         # AI Chat frame
    ai_frame = tk.Frame(left_notebook, bg="#2b2b2b")

    ai_label = tk.Label(ai_frame, text="🤖 AI Chat", bg="#2b2b2b", fg="white", font=("Arial", 10, "bold"))
    ...
    ai_send_btn = tk.Button(ai_input_frame, text="Send", command=self.send_ai_message, bg="#4CAF50", fg="white")
    ai_send_btn.pack(side = "right", padx=(5, 0))

    left_notebook.add(ai_frame, text = "AI Chat")

    #         # Self-Improvement frame
    self_improvement_frame = tk.Frame(left_notebook, bg="#2b2b2b")

    si_label = tk.Label(self_improvement_frame, text="🔧 Self-Improvement", bg="#2b2b2b", fg="white", font=("Arial", 10, "bold"))
    si_label.pack(anchor = "w", padx=5, pady=2)

    #         # Self-Improvement display
    self.si_display = scrolledtext.ScrolledText(
    #             self_improvement_frame,
    wrap = tk.WORD,
    bg = "#1e1e1e",
    fg = "#ffffff",
    height = 15,
    state = "disabled"
    #         )
    self.si_display.pack(fill = "both", expand=True, padx=5, pady=5)

    #         # Self-Improvement control frame
    si_control_frame = tk.Frame(self_improvement_frame, bg="#2b2b2b")
    si_control_frame.pack(fill = "x", padx=5, pady=5)

    si_start_btn = tk.Button(si_control_frame, text="Start Monitoring", command=self.start_self_improvement, bg="#4CAF50", fg="white")
    si_start_btn.pack(side = "left", padx=(0, 5))

    si_stop_btn = tk.Button(si_control_frame, text="Stop Monitoring", command=self.stop_self_improvement, bg="#f44336", fg="white")
    si_stop_btn.pack(side = "left", padx=(0, 5))

    si_clear_btn = tk.Button(si_control_frame, text="Clear", command=self.clear_self_improvement_display, bg="#FF9800", fg="white")
    si_clear_btn.pack(side = "left")

    left_notebook.add(self_improvement_frame, text = "Self-Improve")

    left_paned.add(left_notebook, minsize = 200)

    #         # Store reference to left notebook for tab switching
    self.left_notebook = left_notebook

    main_paned.add(left_paned, minsize = 300)

            # Right panel (editor, terminal)
    right_paned = tk.PanedWindow(main_paned, orient=tk.VERTICAL, bg="#2b2b2b")

    #         # Editor (notebook for tabs)
    editor_frame = tk.Frame(right_paned, bg="#2b2b2b")
    editor_frame.pack(fill = "both", expand=True)

    editor_label = tk.Label(editor_frame, text="📝 Code Editor", bg="#2b2b2b", fg="white", font=("Arial", 10, "bold"))
    editor_label.pack(anchor = "w", padx=5, pady=2)
    ...
    #         # Terminal
    terminal_frame = tk.Frame(right_paned, bg="#2b2b2b")
    terminal_frame.pack(fill = "both", expand=True)

    terminal_label = tk.Label(terminal_frame, text="💻 Terminal", bg="#2b2b2b", fg="white", font=("Arial", 10, "bold"))
    ...
    terminal_prompt = tk.Label(terminal_input_frame, text="$", bg="#2b2b2b", fg="#00ff00", font=("Courier", 10, "bold"))
    terminal_prompt.pack(side = "left")

    self.terminal_input = tk.Entry(terminal_input_frame, bg="#1e1e1e", fg="white", font=("Courier", 10))
    self.terminal_input.pack(side = "left", fill="x", expand=True, padx=(5, 0))
            self.terminal_input.bind("<Return>", self._on_terminal_enter)

    right_paned.add(terminal_frame, minsize = 150)
...
#         """Get currently active text widget with enhanced support."""
#         try:
...
tk.Button(button_frame, text = "Replace All", bg='#4CAF50', fg='white').pack(side='right', padx=5)
...
path = math.divide(self.current_project_path, path)
            self._open_file_in_explorer(str(path))
...
#             # Handle folders - expand/collapse on double-click
            self._toggle_folder_expansion(item_id, path)
#         elif isinstance(path, str) and os.path.isdir(path):
#             # Handle string paths for directories
#             if not os.path.isabs(path):
path = os.path.join(self.current_project_path, path)
            self._toggle_folder_expansion(item_id, Path(path))

#     def _toggle_folder_expansion(self, item_id, folder_path):
#         """Toggle folder expansion state on double-click."""
#         try:
...
self.file_tree.item(item_id, open = False)
                print(f"Collapsed folder: {folder_path}")
#             else:
#                 # Folder is collapsed, expand it
self.file_tree.item(item_id, open = True)
#                 # Load children if not already loaded
                self._add_directory_children(item_id, folder_path)
                print(f"Expanded folder: {folder_path}")

#         except Exception as e:
            print(f"Error toggling folder expansion: {e}")
...
#         # Check if Noodle runtime is available and file should use it
use_noodle_runtime = (
            hasattr(self, 'noodle_runtime') and
#             self.noodle_runtime and
            self.noodle_runtime.is_available() and
            path.endswith('.py') and
            self.config.get('use_noodle_runtime_for_python', False)
#         )

#         def worker():
#             try:
#                 if use_noodle_runtime:
#                     # Use Noodle runtime to execute Python file
#                     logger.info(f"Executing {path} with Noodle runtime")

#                     # Read Python file content
#                     with open(path, 'r', encoding='utf-8') as f:
python_content = f.read()

#                     # Convert Python to NoodleCore on the fly
conversion_result = self.noodle_runtime.execute_noodle_code(
#                         f"# Auto-generated NoodleCore from Python\n# Original Python file: {path}\n\n{python_content}",
#                         {"original_python_file": path, "conversion_context": "auto_execution"}
#                     )

#                     if conversion_result.get('ok', False):
error = conversion_result.get('error', 'Unknown error')
                        self._append_terminal_output(f"[Noodle Runtime Error] {error}\n")
#                     else:
#                         # Execute the converted NoodleCore code
execution_result = self.noodle_runtime.execute_noodle_code(
                            conversion_result.get('data', ''),
#                             {"execution_context": "auto_python_execution"}
#                         )

#                         if execution_result.get('ok', False):
exec_error = execution_result.get('error', 'Unknown error')
                            self._append_terminal_output(f"[Noodle Runtime Error] {exec_error}\n")
#                         else:
                            self._append_terminal_output(f"[Noodle Runtime Output]\n{execution_result.get('data', '')}\n")
#                 else:
...
#         # Ensure Manager AI is first in list if available
#         if self.ai_roles:
...
#         # Update AI role indicator in interface
...
#         except Exception as e:
            return f"[ERROR] Bestandsoperatie mislukt: {str(e)}"
...
#                 return f"[ERROR] Bestand niet gevonden: {file_path}"
...
#             return f"[OK] Bestand gelezen: {file_path}\n```\n{content[:1000]}{'...' if len(content) > 1000 else ''}\n```"

#         except Exception as e:
            return f"[ERROR] Fout bij lezen bestand: {str(e)}"
...
#             return f"[OK] Bestand geschreven: {file_path}"

#         except Exception as e:
            return f"[ERROR] Fout bij schrijven bestand: {str(e)}"
...
#             # Find text widget for this file
text_widget = None
#             for tab_id, info in self.open_files.items():
...
#                 return f"[OK] Bestand gewijzigd: {file_path}"

#             else:
#                 return f"[ERROR] Kon bestand niet openen in editor: {file_path}"

#         except Exception as e:
            return f"[ERROR] Fout bij wijzigen bestand: {str(e)}"
...
result = f"[OK] Projectanalyse (basis): {path}\n\n"
...
#         except Exception as e:
            return f"[ERROR] Fout bij analyseren project: {str(e)}"
...
self.ai_chat.config(state = 'normal')
            ...
            self.ai_chat.insert('end', f"[ERROR] AI Error: {str(e)}\n\n")
self.ai_chat.config(state = 'disabled')
            self.ai_chat.see('end')
...
self.ai_chat.config(state = 'normal')
            self.ai_chat.insert('end', f"You: {message}\n")
#             self.ai_chat.insert('end', "[ERROR] AI Error: Please configure AI Settings with an API key first.\n")
...
#         review_message = f"Please review code in {self.current_file} and provide feedback on code quality, potential issues, and suggestions for improvement. Focus on:\n- Code style and best practices\n- Potential bugs or issues\n- Performance considerations\n- Security concerns\n- Suggestions for improvement"
...
#                 {
#                     "role": "system",
#                     "content": "You are a NoodleCore language expert. Convert given Python code to equivalent NoodleCore (.nc) code. Preserve functionality while using NoodleCore syntax and features."
#                 },
#                 {
#                     "role": "user",
...
#             # Open converted file
            self._open_file_in_explorer(nc_file_path)
...
self.use_noodle_runtime_for_python = config.get('use_noodle_runtime_for_python', False)
...
#                     'model': self.current_ai_model,
#                     'api_key': self.ai_api_key,
#                     'use_noodle_runtime_for_python': self.use_noodle_runtime_for_python
}, f, indent = 2)
...
#         # Noodle Runtime Settings
runtime_frame = tk.Frame(dialog, bg='#2b2b2b', relief='ridge', bd=2)
runtime_frame.pack(fill = 'x', padx=10, pady=10)

tk.Label(runtime_frame, text = "Noodle Runtime Settings", bg='#2b2b2b', fg='white',
font = ("Arial", 10, "bold")).pack(anchor='w', padx=5, pady=(5, 2))

use_noodle_runtime_var = tk.BooleanVar(value=self.use_noodle_runtime_for_python)
#         tk.Checkbutton(runtime_frame, text="Use Noodle runtime for Python files",
variable = use_noodle_runtime_var, bg='#2b2b2b', fg='white',
selectcolor = '#2b2b2b', activebackground='#2b2b2b').pack(anchor='w', padx=5, pady=2)

tk.Label(runtime_frame, text = "When enabled, Python files will be executed using\nthe NoodleCore runtime instead of standard Python.",
bg = '#2b2b2b', fg='gray', justify='left').pack(anchor='w', padx=5, pady=(0, 5))
...
#                         elif line.startswith(' M '):
                            self.terminal_output.insert('end', f"  [EDIT] {line[3:]} (Modified)\n")
#                         elif line.startswith('A '):
                            self.terminal_output.insert('end', f"  [ADD] {line[3:]} (Added)\n")
#                         elif line.startswith('D '):
                            self.terminal_output.insert('end', f"  [DEL] {line[3:]} (Deleted)\n")
#                         else:
                            self.terminal_output.insert('end', f"  {line}\n")
#                 else:
                    self.terminal_output.insert('end', "[OK] Working directory is clean\n")
#             else:
                    self.terminal_output.insert('end', "[ERROR] Not a Git repository or Git error\n")

#         except FileNotFoundError:
            self.terminal_output.insert('end', "[ERROR] Git not found. Please install Git.\n")
#         except Exception as e:
...
#         # Panel is already visible in left notebook
#         # Switch to Self-Improve tab
#         if hasattr(self, 'left_notebook'):
#             try:
#                 # Find Self-Improve tab index
...
#                         return
                messagebox.showinfo("Self-Improvement", "Self-Improvement panel is available in left sidebar")
#             except Exception as e:
                print(f"Error switching to Self-Improvement panel: {e}")

#     def clear_self_improvement_display(self):
#         """Clear the self-improvement display."""
#         if hasattr(self, 'si_display'):
self.si_display.config(state = 'normal')
            self.si_display.delete('1.0', 'end')
self.si_display.config(state = 'disabled')
self.status_bar.config(text = "Self-Improvement display cleared")
...
#         # Title
        tk.Label(
#             settings_dialog,
text = "Self-Improvement Settings",
bg = '#2b2b2b',
fg = 'white',
font = ('Arial', 14, 'bold')
).pack(pady = 10)
...
#     def _setup_ide_logging(self) -> None:
...
#     def _init_aere_integration(self) -> None:
        """Initialize AERE (AI Error Resolution Engine) integration."""
#         try:
...
#     def _update_project_index(self, ai_command=None):
#         """Update project index for AI context."""
#         try:
#             if not ai_command:
#                 # Full project index update
self.project_index = {}
...
#                             # Only index text files that are reasonably sized
...
#             else:
#                 # Update index for specific file operation
#                 if ai_command['type'] in ['read', 'write', 'modify']:
...
#                     if ai_command['type'] == 'write' or ai_command['type'] == 'modify':
#                         # Add/update file in index
self.project_index[rel_path] = ai_command.get('content', '')
#                     elif ai_command['type'] == 'read' and os.path.exists(file_path):
#                         # Read and add file to index
#                         try:
#                             with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
self.project_index[rel_path] = f.read()
#                         except Exception as e:
                            print(f"Failed to index {rel_path}: {e}")
...
#     # ---------------------------
#     # TRM Controller Methods
#     # ---------------------------

#     def show_trm_controller_status(self):
#         """Show TRM controller status dialog."""
#         if not hasattr(self, 'trm_controller_integration') or not self.trm_controller_integration:
            messagebox.showinfo("TRM Controller", "TRM Controller integration is not available")
#             return

#         try:
status = self.trm_controller_integration.get_integration_status()
controller_status = self.trm_controller_integration.get_controller_status()

#             # Create status dialog
dialog = tk.Toplevel(self.root)
            dialog.title("TRM Controller Status")
...
#             # Status frame
status_frame = tk.Frame(dialog, bg='#1e1e1e', relief='raised', bd=1)
...
controller_text = f"Running: {status.get('is_controller_running', False)}"
            tk.Label(
#                 status_frame,
text = controller_text,
bg = '#1e1e1e',
#                 fg='#00ff00' if status.get('is_controller_running', False) else '#ff0000',
font = ('Arial', 10)
...
#             # Current cycle
            tk.Label(
#                 status_frame,
text = f"Current Cycle: {status.get('current_cycle', 0)}",
bg = '#1e1e1e',
fg = 'white',
font = ('Arial', 10)
).pack(anchor = 'w', padx=20, pady=2)
...
#             # Systems status
systems_status = controller_status.get('systems_status', {})
            tk.Label(
#                 status_frame,
text = "Systems Status:",
bg = '#1e1e1e',
...
#                 status_icon = "✅" if system_status == "healthy" else "❌"
                tk.Label(
#                     status_frame,
text = f"  {status_icon} {system_name}: {system_status}",
bg = '#1e1e1e',
fg = 'white',
font = ('Arial', 9)
).pack(anchor = 'w', padx=20, pady=2)
...
#         except Exception as e:
            messagebox.showerror("Error", f"Failed to get TRM Controller status: {str(e)}")

#     def start_trm_controller(self):
#         """Start TRM controller."""
#         if not hasattr(self, 'trm_controller_integration') or not self.trm_controller_integration:
            messagebox.showinfo("TRM Controller", "TRM Controller integration is not available")
#             return
...
success = self.trm_controller_integration.start_controller()
#             if success:
                messagebox.showinfo("TRM Controller", "TRM Controller started successfully")
self.status_bar.config(text = "TRM Controller started")
#             else:
                messagebox.showerror("Error", "Failed to start TRM Controller")
#         except Exception as e:
            messagebox.showerror("Error", f"Failed to start TRM Controller: {str(e)}")

#     def stop_trm_controller(self):
#         """Stop TRM controller."""
#         if not hasattr(self, 'trm_controller_integration') or not self.trm_controller_integration:
            messagebox.showinfo("TRM Controller", "TRM Controller integration is not available")
#             return
...
#         try:
success = self.trm_controller_integration.stop_controller()
#             if success:
                messagebox.showinfo("TRM Controller", "TRM Controller stopped successfully")
self.status_bar.config(text = "TRM Controller stopped")
#             else:
                messagebox.showerror("Error", "Failed to stop TRM Controller")
#         except Exception as e:
            messagebox.showerror("Error", f"Failed to stop TRM Controller: {str(e)}")

#     def trigger_trm_cycle(self):
#         """Trigger immediate TRM cycle."""
#         if not hasattr(self, 'trm_controller_integration') or not self.trm_controller_integration:
            messagebox.showinfo("TRM Controller", "TRM Controller integration is not available")
#             return

#         try:
success = self.trm_controller_integration.trigger_immediate_cycle()
#             if success:
                messagebox.showinfo("TRM Controller", "Immediate TRM cycle triggered")
self.status_bar.config(text = "TRM cycle triggered")
#             else:
                messagebox.showerror("Error", "Failed to trigger TRM cycle")
#         except Exception as e:
            messagebox.showerror("Error", f"Failed to trigger TRM cycle: {str(e)}")

#     def show_trm_settings(self):
#         """Show TRM Controller settings dialog."""
#         if not hasattr(self, 'trm_controller_integration') or not self.trm_controller_integration:
            messagebox.showinfo("TRM Controller", "TRM Controller integration is not available")
#             return
...
#         # Create settings dialog
dialog = tk.Toplevel(self.root)
        dialog.title("TRM Controller Settings")
        dialog.geometry("500x400")
dialog.configure(bg = '#2b2b2b')
        dialog.transient(self.root)
        dialog.grab_set()
...
#         # Title
        tk.Label(
#             dialog,
text = "⚙️ TRM Controller Settings",
bg = '#2b2b2b',
fg = 'white',
font = ('Arial', 14, 'bold')
).pack(pady = 10)
...
#         # Settings frame
settings_frame = tk.Frame(dialog, bg='#1e1e1e', relief='raised', bd=1)
settings_frame.pack(fill = 'both', expand=True, padx=10, pady=5)

#         # Get current settings
integration_status = self.trm_controller_integration.get_integration_status()
current_config = integration_status.get('config', {})

#         # Auto-start setting
auto_var = tk.BooleanVar(value=current_config.get('auto_start_controller', False))
        tk.Checkbutton(
#             settings_frame,
text = "Auto-start TRM Controller on IDE launch",
variable = auto_var,
bg = '#1e1e1e',
fg = 'white',
selectcolor = '#2b2b2b'
...
#         # Monitoring interval
        tk.Label(
#             settings_frame,
text = "Status Monitoring Interval (seconds):",
bg = '#1e1e1e',
fg = 'white',
font = ('Arial', 10)
).pack(anchor = 'w', padx=10, pady=(10, 0))

interval_var = tk.IntVar(value=current_config.get('monitoring_interval', 2.0))
interval_entry = tk.Entry(settings_frame, textvariable=interval_var, bg='#1e1e1e', fg='white')
...
#             try:
new_config = {
                    'auto_start_controller': auto_var.get(),
                    'monitoring_interval': interval_var.get()
#                 }
                self.trm_controller_integration.configure_settings(new_config)
                messagebox.showinfo("Settings", "TRM Controller settings saved successfully")
                dialog.destroy()
#             except Exception as e:
