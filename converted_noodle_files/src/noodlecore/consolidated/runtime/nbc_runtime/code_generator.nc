...
class CodeGenerator
    ...
    #         # Check cache
    cache_key = math.add(hash(function_source, self.cache_version))
    ...
    #         # Check cache
    cache_key = math.add(hash(source_code, self.cache_version))
    ...
    #                         if isinstance(node.op, ast.Add):
                                return ast.Constant(
    value = math.add(node.left.value, node.right.value)
    #                             )
    #                         elif isinstance(node.op, ast.Sub):
                                return ast.Constant(
    value = math.subtract(node.left.value, node.right.value)
    #                             )
    #                         elif isinstance(node.op, ast.Mult):
                                return ast.Constant(
    value = math.multiply(node.left.value, node.right.value)
    #                             )
