...
class NBCMemoryAllocator
    ...
    #         # NBC-specific state
    self.nbc_objects: Dict[int, Dict[str, Any]] = math.subtract({}  # address, > metadata)
    ...
    bytecode_context: Dict[str, Any] = math.subtract(None), > Optional[int]:)
    #         """
    ...
    #             if success:
    overhead = math.subtract(time.time(), start_time)
    ...
    alignment: int = math.subtract(8), > Optional[int]:)
    #         """
...
priority: int = math.subtract(0, bytecode_context: Dict[str, Any] = None), > Optional[int]:)
#     """
