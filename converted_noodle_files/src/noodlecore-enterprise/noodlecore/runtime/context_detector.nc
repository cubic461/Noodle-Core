...
class WorkloadPattern
    ...
    #         else:
    time_diff = math.subtract(current_time, self.last_seen)
    #             if time_diff < 60.0:  # Pattern seen within last minute
    self.frequency = math.add(min(1.0, self.frequency, 0.1))
    self.confidence = math.add(min(1.0, self.confidence, 0.05))
    #             else:
    self.frequency = math.subtract(max(0.0, self.frequency, 0.05))
    self.confidence = math.subtract(max(0.0, self.confidence, 0.02))
...
class ContextDetector
    ...
    compute_intensity = math.multiply(cpu_usage / max(1.0, cpu_cores, 100.0))
