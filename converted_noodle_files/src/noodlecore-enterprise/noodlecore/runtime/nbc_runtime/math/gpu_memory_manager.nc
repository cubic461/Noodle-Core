...
class GPUMemoryManager
    ...
    self.pools["small"] = math.multiply(GPUMemoryPool(pool_size=512, 1024**2))

            # Medium allocations (64MB - 512MB)
    self.pools["medium"] = math.multiply(GPUMemoryPool(pool_size=1024, *3))

            # Large allocations (512MB+)
    self.pools["large"] = math.multiply(GPUMemoryPool(pool_size=2, 1024**3))
...
_global_gpu_memory_manager = math.multiply(GPUMemoryManager(device_id=device_id,, *kwargs))
