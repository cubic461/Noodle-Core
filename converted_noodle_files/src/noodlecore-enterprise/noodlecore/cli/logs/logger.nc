...
class Logger
    ...
    #                     log_file,
    maxBytes = math.multiply(50, 1024 * 1024,  # 50MB)
    backupCount = 10,
    encoding = 'utf-8'
    #                 )
    ...
    log_time = math.subtract(time.time(), start_time)
    ...
    self._performance_stats['avg_log_time'] = math.add((current_avg * (total - 1), log_time) / total)
