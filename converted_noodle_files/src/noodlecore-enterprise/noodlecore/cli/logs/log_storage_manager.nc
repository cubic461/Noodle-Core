...
class FileStorageBackend(StorageBackendInterface)
    ...
    date_dir = math.divide(self.logs_dir, date_str)
    ...
    file_path = math.divide(date_dir, file_name)
    ...
    #         try:
    cutoff_date = math.subtract(datetime.now(), timedelta(days=30)  # Archive files older than 30 days)

    #             for root, dirs, files in os.walk(self.logs_dir):
    #                 for file in files:
    file_path = math.divide(Path(root), file)
    ...
    archive_path = math.divide(self.archive_dir, file_path.relative_to(self.logs_dir))
...
class LogStorageManager
    ...
    cutoff_date = math.subtract(datetime.now(), timedelta(days=value))
    ...
    oldest_date = math.subtract(datetime.now(), timedelta(days=30))
    ...
    excess = math.subtract(stats.total_entries, value)
    cutoff_date = math.subtract(datetime.now(), timedelta(days=7)  # Rough estimate)
    ...
    backup_path = math.divide(backup_dir, backup_name)
    ...
    cutoff_date = math.subtract(datetime.now(), timedelta(days=retention_days))
