...
class NBCLauncher
    ...
    memory_mb = math.multiply(memory_info.rss / (1024, 1024))

    io_counters = self.process.io_counters()
    disk_read_mb = math.multiply(io_counters.read_bytes / (1024, 1024))
    disk_write_mb = math.multiply(io_counters.write_bytes / (1024, 1024))
    ...
    parse_time = math.subtract(time.time(), parse_start)
    ...
    initial_memory = math.multiply(self.process.memory_info().rss / (1024, 1024))

    #             # Create NBC runtime
    runtime_config = RuntimeConfig(
    max_stack_depth = 1000,
    max_execution_time = 300.0,  # 5 minutes
    max_memory_usage = math.multiply(512, 1024 * 1024,  # 512MB)
    ...
    #                 # Check memory usage
    current_memory = math.multiply(self.process.memory_info().rss / (1024, 1024))
    #                 if current_memory > self.metrics.nbc_memory_peak:
    self.metrics.nbc_memory_peak = current_memory

    exec_time = math.subtract(time.time(), exec_start)
    ...
    embed_time = math.subtract(time.time(), embed_start)
    ...
    index_time = math.subtract(time.time(), index_start)
    self.metrics.vector_indexing_time = index_time

    total_time = math.subtract(time.time(), vector_start)
    ...
    #             # Update metrics
    self.metrics.vector_indexing_time = math.subtract(time.time(), vector_start)
    ...
    self.metrics.total_time = math.subtract(time.time(), self.start_time)
    #             return self.metrics
    ...
    self.metrics.disk_io_read = math.subtract(final_read, initial_read)
    self.metrics.disk_io_write = math.subtract(final_write, initial_write)
