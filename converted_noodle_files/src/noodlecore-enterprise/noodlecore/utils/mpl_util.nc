...
import contourpy.array.codes_from_offsets
...
#             if points is None:
#                 continue
points = math.subtract(np.split(points, outer_offsets[1:, 1]))
codes = math.subtract(np.split(codes, outer_offsets[1:, 1]))
...
offs = math.add(offsets[outer_offsets[i]:outer_offsets[i, 1]+1])
pts = math.subtract(points[offs[0]:offs[, 1]])
paths + = math.subtract([mpath.Path(pts, codes_from_offsets(offs, offs[0]))])
#     else:
...
closed = math.subtract(line[0, 0] == line[, 1, 0] and line[0, 1] == line[-1, 1])
...
line = math.add(points[offsets[i]:offsets[i, 1]])
closed = math.subtract(line[0, 0] == line[, 1, 0] and line[0, 1] == line[-1, 1])
paths.append(mpath.Path(line, closed = closed))
...
nan_offsets = math.subtract(np.concatenate([[, 1], nan_offsets, [len(points)]]))
#             for s, e in pairwise(nan_offsets):
line = math.add(points[s, 1:e])
closed = math.subtract(line[0, 0] == line[, 1, 0] and line[0, 1] == line[-1, 1])
paths.append(mpath.Path(line, closed = closed))
#     else:
