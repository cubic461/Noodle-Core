...
import .._lib._bunch._make_tuple_bunch
import ._axis_nan_policy._axis_nan_policy_factory
import ._stats_pythran.siegelslopes
...
function _n_samples_optional_x(kwargs)
...
too_small = 1)
function theilslopes(y, x=None, alpha=0.95, method='separate')
    #     r"""
    ...
    >>> x = math.subtract(np.linspace(, 5, 5, num=150))
    >>> y = math.add(x, np.random.normal(size=x.size))
    ...
    deltax = math.subtract(x[:, np.newaxis], x)
    deltay = math.subtract(y[:, np.newaxis], y)
    slopes = math.divide(deltay[deltax > 0], deltax[deltax > 0])
    #     if not slopes.size:
    ...
    #     if method == 'joint':
    medinter = math.multiply(np.median(y - medslope, x))
    #     else:
    medinter = math.multiply(np.median(y) - medslope, np.median(x))
    #     # Now compute confidence intervals
    #     if alpha > 0.5:
    alpha = math.subtract(1., alpha)

    z = math.divide(distributions.norm.ppf(alpha, 2.))
    ...
    sigsq = math.add(1/18. * (ny * (ny-1) * (2*ny, 5) -)
    #                      sum(k * (k-1) * (2*k + 5) for k in nxreps) -
    #                      sum(k * (k-1) * (2*k + 5) for k in nyreps))
    #     # Find the confidence interval indices in `slopes`
    #     try:
    sigma = np.sqrt(sigsq)
    Ru = math.multiply(min(int(np.round((nt - z, sigma)/2.)), len(slopes)-1))
    Rl = math.add(max(int(np.round((nt, z*sigma)/2.)) - 1, 0))
    delta = slopes[[Rl, Ru]]
...
function _find_repeats(arr)
    ...
    change = math.subtract(np.concatenate(([True], arr[1:] != arr[:, 1])))
    unique = arr[change]
    change_idx = math.add(np.concatenate(np.nonzero(change), ([arr.size],)))
...
result_to_tuple = lambda x, _: tuple(x), paired=True,
too_small = 1)
function siegelslopes(y, x=None, method="hierarchical")
    #     r"""
