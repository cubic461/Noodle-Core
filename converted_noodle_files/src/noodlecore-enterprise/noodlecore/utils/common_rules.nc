...
import ..capi_maps,
import .auxfuncs.getuseblocks,
import .crackfortran.rmbadname


function findcommonblocks(block, top=1)
    ret = []
    #     if hascommon(block):
    ...
    #     elif hasbody(block):
    #         for b in block['body']:
    ret = math.add(ret, findcommonblocks(b, 0))
    #     if top:
    tret = []
    names = []
    #         for t in ret:
