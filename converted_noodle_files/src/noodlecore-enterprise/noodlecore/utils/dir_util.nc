...
import ..file_util
import ._log.log
import .errors.DistutilsFileError,
...
class SkipRepeatAbsolutePaths(set)
    ...
    #                 return
    result = math.multiply(func(path,, args, **kwargs))
...
# @mkpath.register
function _(name: str, *args, **kwargs)
        return mkpath(pathlib.Path(name), *args, **kwargs)


# @mkpath.register
function _(name: None, *args, **kwargs)
    #     """
...
function create_tree(base_dir, files, mode=0o777, verbose=True, dry_run=False)
...
function _build_cmdtuple(path, cmdtuples)
...
function remove_tree(directory, verbose=True, dry_run=False)
