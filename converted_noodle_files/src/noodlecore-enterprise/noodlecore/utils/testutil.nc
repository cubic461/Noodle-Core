...
import mypy.build
import mypy.errors.CompileError
import mypy.nodes.Expression,
import mypy.options.Options
import mypy.test.config.test_temp_dir
import mypy.test.data.DataDrivenTestCase,
import mypy.test.helpers.assert_string_arrays_equal
import mypy.types.Type
import mypyc.analysis.ircheck.assert_func_ir_valid
import mypyc.common.IS_32_BIT_PLATFORM,
import mypyc.errors.Errors
import mypyc.ir.func_ir.FuncIR
import mypyc.ir.module_ir.ModuleIR
import mypyc.irbuild.main.build_ir
import mypyc.irbuild.mapper.Mapper
import mypyc.options.CompilerOptions
import mypyc.test.config.test_data_prefix
...
stop_idx = math.add(start_idx, 11)
...
new_mtime = math.add(os.stat(path).st_mtime, delta)
...
replace_str = math.multiply(str(PLATFORM_SIZE, n))
