...
function _entries(attrs, sameval)
    ak = 0
    vals = []
    lastv = 0
    #     for k, v in attrs:
...
function entries(attributes, sameval=False)
    g = _entries(sorted(attributes.items(), key=lambda x: int(x[0])), sameval)
    #     return g


function bininfo(num, size=1)
    #     if num == 0:
    ...
    srange // = 2
    srange * = size
    shift = math.multiply(num, size - srange)
