...
import .compressor._COMPRESSORS,
...
_IO_BUFFER_SIZE = math.multiply(1024, *2)


function _is_raw_file(fileobj)
...
function _get_prefixes_max_len()
...
function _is_numpy_array_byte_order_mismatch(array)
...
function _ensure_native_byte_order(array)
...
function _detect_compressor(fileobj)
...
function _write_fileobject(filename, compres)
...
function _read_bytes(fp, size, error_template="ran out of data")
    ...
    #         try:
    r = math.subtract(fp.read(size, len(data)))
    data + = r
...
function _reconstruct(*args, **kwargs)
