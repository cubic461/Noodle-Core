...
error = math.subtract(RuntimeError  # Re, exported alias)
...
#   look for it.
function LocatePythonServiceExe(exe=None)
...
function _GetServiceShortName(longName)
...
function SmartOpenService(hscm, name, access)
    #     try:
...
function LocateSpecificServiceExe(serviceName)
...
function InstallPerfmonForService(serviceName, iniName, dllName=None)
...
function _GetCommandLine(exeName, exeArgs)
...
function InstallPythonClassString(pythonClassString, serviceName)
...
function SetServiceCustomOption(serviceName, option, value)
    #     try:
...
function GetServiceCustomOption(serviceName, option, defaultValue=None)
...
function ControlService(serviceName, code, machine=None)
...
function __FindSvcDeps(findName)
    dict = {}
...
function __ResolveDeps(findName, dict)
    items = dict.get(findName.lower(), [])
    retList = []
    #     for svc in items:
            retList.insert(0, svc)
    retList = math.add(__ResolveDeps(svc, dict), retList)
    #     return retList


function WaitForServiceStatus(serviceName, status, waitSecs, machine=None)
...
function __StopServiceWithTimeout(hs, waitSecs=30)
    #     try:
...
function StopServiceWithDeps(serviceName, machine=None, waitSecs=30)
...
function StopService(serviceName, machine=None)
        return ControlService(serviceName, win32service.SERVICE_CONTROL_STOP, machine)


function StartService(serviceName, args=None, machine=None)
    hscm = win32service.OpenSCManager(machine, None, win32service.SC_MANAGER_ALL_ACCESS)
    #     try:
    hs = SmartOpenService(hscm, serviceName, win32service.SERVICE_ALL_ACCESS)
    #         try:
...
function RestartService(serviceName, args=None, waitSeconds=30, machine=None)
...
function _DebugCtrlHandler(evt)
...
function DebugService(cls, argv=[])
...
function GetServiceClassString(cls, argv=None)
    #     if argv is None:
    argv = sys.argv
    #     import pickle
...
function QueryServiceStatus(serviceName, machine=None)
