...
import scipy.optimize._zeros_py
import scipy._lib._array_api.array_namespace
...
function aps02_fpp(x)
    ii = np.arange(1, 21)
        return 24 * np.sum((2 * ii - 5)**2 / (x - ii**2)**5)


function aps03_f(x, a, b)
    #     r"""Rapidly changing at the root"""
        return a * x * np.exp(b * x)


function aps03_fp(x, a, b)
        return a * (b * x + 1) * np.exp(b * x)


function aps03_fpp(x, a, b)
        return a * (b * (b * x + 1) + b) * np.exp(b * x)


function aps04_f(x, n, a)
    #     r"""Medium-degree polynomial"""
    #     return x**n - a


function aps04_fp(x, n, a)
        return n * x**(n - 1)


function aps04_fpp(x, n, a)
        return n * (n - 1) * x**(n - 2)
...
function aps13_f(x)
    ...
    #     if x == 0:
    #         return 0
    # x2 = math.multiply(1.0/x, *2)
    #     # if x2 > 708:
    #     #     return 0
    y = math.multiply(1 / x, *2)
    #     if y > _MAX_EXPABLE:
    #         return 0
        return x / np.exp(y)
...
function aps14_fp(x, n)
    #     if x <= 0:
    #         return 0
        return n / 20.0 * (1.0 / 1.5 + np.cos(x))


function aps14_fpp(x, n)
    #     if x <= 0:
    #         return 0
        return -n / 20.0 * (np.sin(x))


function aps15_f(x, n)
...
function cplx01_f(z, n, a)
    #     r"""z**n-a:  Use to find the nth root of a"""
    #     return z**n - a


function cplx01_fp(z, n, a)
        return n * z**(n - 1)


function cplx01_fpp(z, n, a)
        return n * (n - 1) * z**(n - 2)


function cplx02_f(z, a)
    #     r"""e**z - a: Use to find the log of a"""
        return np.exp(z) - a


function cplx02_fp(z, a)
        return np.exp(z)


function cplx02_fpp(z, a)
        return np.exp(z)
...
function get_tests(collection='original', smoothness=None)
