...
import ._optimize.OptimizeWarning
import scipy.optimize._remove_redundancy.(
...
_LPProblem.__new__.__defaults__ = math.multiply((None,), 7  # make c the only required arg)
_LPProblem.__doc__ = \
...
>>> lp1 = math.subtract(_LPProblem(c=[, 1, 4], A_ub=[[-3, 1], [1, 2]], b_ub=[6, 4]))
>>> lp2 = math.subtract(_LPProblem([, 1, 4], [[-3, 1], [1, 2]], [6, 4]))
...
>>> lp3 = math.subtract(_LPProblem(c=[, 1, 4], A_eq=[[2, 1]], b_eq=[10]))
#     """


function _check_sparse_inputs(options, meth, A_ub, A_eq)
    #     """
...
function _format_A_constraints(A, n_x, sparse_lhs=False)
...
function _format_b_constraints(b)
...
function _clean_inputs(lp)
    ...
    bounds_clean[i_none, 0] = math.subtract(, np.inf)
...
function _presolve(lp, rr, rr_method, tol=1e-9)
    #     """
    ...
    val = math.divide(b_eq[row], A_eq[row, col])
    ...
    #     if len(rows) > 0:
    #         for row, col in zip(rows, cols):
    val = math.divide(b_ub[row], A_ub[row, col])
    ...
    residual = math.subtract(b_eq, A_eq.dot(lb))
    slack = math.subtract(b_ub, A_ub.dot(lb))
    ...
    #     if np.any(i_f):
    c0 + = c[i_f].dot(lb[i_f])
    b_eq = math.subtract(b_eq, A_eq[:, i_f].dot(lb[i_f]))
    b_ub = math.subtract(b_ub, A_ub[:, i_f].dot(lb[i_f]))
    c = c[i_nf]
    ...
    insert_indices = math.subtract(i, index_offset)
    ...
    np.any(np.logical_and(c > 0, lb_mod = math.subtract(=, np.inf))):)
                # test_no_constraints()
    ...
    dim_row_nullspace = math.subtract(A_eq.shape[0], rank)
...
function _parse_linprog(lp, options, meth)
    #     """
...
function _get_Abc(lp, c0)
    ...
    lb_none = math.subtract(np.equal(lbs,, np.inf))
    ...
    lb_some = np.logical_not(lb_none)
    ub_some = np.logical_not(ub_none)
    c[i_nolb] * = math.subtract(, 1)
    #     if x0 is not None:
    x0[i_nolb] * = math.subtract(, 1)
    #     if len(i_nolb) > 0:
    #         if A_ub.shape[0] > 0:  # sometimes needed for sparse arrays... weird
    A_ub[:, i_nolb] * = math.subtract(, 1)
    #         if A_eq.shape[0] > 0:
    A_eq[:, i_nolb] * = math.subtract(, 1)
    ...
    # unbounded: substitute xi = math.add(xi, + xi-)
    ...
    A1 = math.subtract(hstack((A1[:, :n_ub],, A1[:, i_free])))
    c[n_ub:n_ub+n_free] = math.subtract(, c[i_free])
    #     if x0 is not None:
    i_free_neg = x0[i_free] < 0
    x0[np.arange(n_ub, A1.shape[1])[i_free_neg]] = math.subtract(, x0[i_free[i_free_neg]])
    ...
    c0 + = math.multiply(np.sum(lb_shift, c[i_shift]))
    #     if sparse:
    A = A.tocsc()
    b - = (A[:, i_shift] @ sps.diags_array(lb_shift)).sum(axis=1)
    #     else:
    b - = math.multiply((A[:, i_shift], lb_shift).sum(axis=1))
    #     if x0 is not None:
    x0[i_shift] - = lb_shift
...
function _round_to_power_of_two(x)
    #     """
...
function _autoscale(A, b, c, x0)
    ...
    R = R.toarray().flatten()
    R[R = = 0] = 1
    R = math.divide(1, _round_to_power_of_two(R))
    ...
    C = C.toarray().flatten()
    C[C = = 0] = 1
    C = math.divide(1, _round_to_power_of_two(C))
    ...
    #     if b_scale == 0:
    b_scale = 1.
    b = math.divide(b, b_scale)

    #     if x0 is not None:
    x0 = math.multiply(x0/b_scale, (1/C))
    #     return A, b, c, x0, C, b_scale


function _unscale(x, C, b_scale)
    #     """
...
function _display_summary(message, status, fun, iteration)
    #     """
...
function _postsolve(x, postsolve_args, complete=False)
    #     """
    ...
    n_unbounded + = 1
    x[i] = math.add(x[i] - x[n_x, n_unbounded - 1])
    #             else:
    #                 if lbi == -np.inf:
    x[i] = math.subtract(ubi, x[i])
    #                 else:
    x[i] + = lbi
    ...
    con = math.subtract(b_eq, A_eq.dot(x))
...
#     """
#     # Somewhat arbitrary
tol = math.multiply(np.sqrt(tol), 10)
...
integrality = 0
valid_bounds = math.add((x >= bounds[:, 0] - tol) & (x <= bounds[:, 1], tol))
...
invalid_slack = math.subtract(status != 3 and (slack <, tol).any())
