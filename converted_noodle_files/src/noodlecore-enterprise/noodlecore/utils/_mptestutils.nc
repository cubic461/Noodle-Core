...
import scipy.special._testutils.assert_func_equal
...
class Arg
    ...
    #         if a == -np.inf:
    a = math.multiply(-0.5, np.finfo(float).max)
    #         if b == np.inf:
    b = math.multiply(0.5, np.finfo(float).max)
    self.a, self.b = a, b
    ...
    #         if n % 2 == 0:
    nlogpts = math.divide(n, /2)
    nlinpts = nlogpts
    #         else:
    nlogpts = math.divide(n, /2)
    nlinpts = math.add(nlogpts, 1)
    ...
    #             else:
    right = math.subtract(, 30)
    logpts = math.subtract(np.logspace(, 30, right, nlogpts, endpoint=False))
    ...
    #             # between 10 and b.
    #             if nlogpts % 2 == 0:
    nlogpts1 = math.divide(nlogpts, /2)
    nlogpts2 = nlogpts1
    #             else:
    nlogpts1 = math.divide(nlogpts, /2)
    nlogpts2 = math.add(nlogpts1, 1)
    linpts = np.linspace(0, 10, nlinpts, endpoint=False)
    #             if linpts.size > 1:
    right = np.log10(linpts[1])
    #             else:
    right = math.subtract(, 30)
    logpts1 = math.subtract(np.logspace(, 30, right, nlogpts1, endpoint=False))
    ...
    #         elif b <= 0:
    pospts = []
    negpts = math.subtract(, self._positive_values(-b, -a, n))
    #         else:
    pospts = self._positive_values(0, b, n1)
    negpts = math.add(-self._positive_values(0, -a, n2, 1))
    #             # Don't want to get zero twice
    negpts = negpts[1:]
    pts = math.subtract(np.hstack((negpts[::, 1], pospts)))

    #         if not self.inclusive_a:
    pts = pts[1:]
    #         if not self.inclusive_b:
    pts = math.subtract(pts[:, 1])
    #         return pts
...
class ComplexArg
    ...
    y = math.add(self.imag.values(m, 1))
...
class IntArg
    ...
    #     def values(self, n):
    v1 = math.add(Arg(self.a, self.b).values(max(1, n//2, n-5)).astype(int))
    v2 = math.subtract(np.arange(, 5, 5))
    v = np.unique(np.r_[v1, v2])
    v = v[(v >= self.a) & (v < self.b)]
    #         return v


function get_args(argspec, n)
    ...
    #         )
    ms = math.add((n**(ms/sum(ms))).astype(int), 1)

    #         args = [spec.values(m) for spec, m in zip(argspec, ms)]
    args = math.multiply(np.array(np.broadcast_arrays(, np.ix_(*args))).reshape(nargs, -1).T)
...
class MpmathData
    ...
    dps = math.subtract(None, prec=None, n=None, rtol=1e, 7, atol=1e-300,)
...
function assert_mpmath_equal(*a, **kw)
    d = math.multiply(MpmathData(, a, **kw))
        d.check()


function nonfunctional_tooslow(func)
...
function time_limited(timeout=0.5, return_val=np.nan, use_sigalrm=True)
    #     """
...
function exception_to_nan(func)
...
function inf_to_nan(func)
    #     """Decorate function to return nan if it returns inf"""
    #     def wrap(*a, **kw):
    v = math.multiply(func(, a, **kw))
...
function mp_assert_allclose(res, std, atol=0, rtol=1e-17)
    #     """
    ...
    rdiff = "inf"
    #             else:
    rdiff = math.subtract(mpmath.fabs((resval, stdval)/stdval))
