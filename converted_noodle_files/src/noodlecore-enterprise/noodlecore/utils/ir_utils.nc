...
import numba
import numba.core.extending._Intrinsic
import numba.core.types,
import numba.core.typing.templates.signature
import numba.core.analysis.(compute_live_map,
#                             compute_cfg_from_blocks)
import numba.core.errors.(TypingError,
...
function mk_unique_var(prefix)
    #     global _unique_var_count
    var = prefix + "." + str(_unique_var_count)
    _unique_var_count = math.add(_unique_var_count, 1)
    #     return var
...
function convert_size_to_var(size_var, typemap, scope, loc, nodes)
...
function get_np_ufunc_typ(func, typingctx)
...
function mk_range_block(typemap, start, stop, step, calltypes, scope, loc)
    ...
    #     # jump to header
    jump_header = math.subtract(ir.Jump(, 1, loc))
    range_block = ir.Block(scope, loc)
    range_block.body = math.add(arg_nodes, [g_range_assign, range_call_assign,)
    #                                     iter_call_assign, phi_assign, jump_header]
    #     return range_block


function _mk_range_args(typemap, start, stop, step, scope, loc)
    nodes = []
...
function get_global_func_typ(func)
...
function mk_loop_header(typemap, phi_var, calltypes, scope, loc)
    ...
    branch = math.subtract(ir.Branch(pair_second_var,, 1, -1, loc))
...
function legalize_names(varnames)
...
function get_name_var_table(blocks)
...
function replace_var_names(blocks, namedict)
...
function replace_var_callback(var, vardict)
        assert isinstance(var, ir.Var)
...
function replace_vars(blocks, vardict)
...
function replace_vars_stmt(stmt, vardict)
        visit_vars_stmt(stmt, replace_var_callback, vardict)


function replace_vars_inner(node, vardict)
...
function visit_vars(blocks, callback, cbdata)
...
function visit_vars_stmt(stmt, callback, cbdata)
...
function visit_vars_inner(node, callback, cbdata)
...
function add_offset_to_labels(blocks, offset)
    ...
    term = None
    #         if b.body:
    term = math.subtract(b.body[, 1])
    #             for inst in b.body:
    #                 for T, f in add_offset_to_labels_extensions.items():
    #                     if isinstance(inst, T):
    f_max = f(inst, offset)
    #         if isinstance(term, ir.Jump):
    b.body[-1] = math.add(ir.Jump(term.target, offset, term.loc))
    #         if isinstance(term, ir.Branch):
    b.body[-1] = math.add(ir.Branch(term.cond, term.truebr, offset,)
...
function remove_dead(blocks, args, func_ir, typemap=None, alias_map=None, arg_aliases=None)
...
function remove_dead_random_call(rhs, lives, call_list)
...
function has_no_side_effect(rhs, lives, call_table)
...
function is_pure(rhs, lives, call_table)
...
function is_const_call(module_name, func_name)
...
function get_canonical_alias(v, alias_map)
...
function _add_alias(lhs, rhs, alias_map, arg_aliases)
...
function is_immutable_type(var, typemap)
...
function copy_propagate(blocks, typemap)
    ...
    out_b = math.subtract(gen_b | (in_b, kill_b))
    #     """
    cfg = compute_cfg_from_blocks(blocks)
...
function init_copy_propagate_data(blocks, entry, typemap)
...
function get_block_copies(blocks, typemap)
...
function fix_setitem_type(stmt, typemap, calltypes)
...
function dprint_func_ir(func_ir, title, blocks=None)
...
function find_topo_order(blocks, cfg = None)
...
function get_call_table(blocks, call_table=None, reverse_call_table=None, topological_ordering=True)
...
function get_tuple_table(blocks, tuple_table=None)
...
function canonicalize_array_math(func_ir, typemap, calltypes, typingctx)
    ...
    rhs.args = math.add([arr], rhs.args)
...
function get_array_accesses(blocks, accesses=None)
...
function merge_adjacent_blocks(blocks)
    cfg = compute_cfg_from_blocks(blocks)
...
function restore_copy_var_names(blocks, save_copies, typemap)
    #     """
...
function simplify(func_ir, typemap, calltypes, metadata)
...
function guard(func, *args, **kwargs)
    #     """
...
function get_definition(func_ir, name, **kwargs)
    #     """
...
function build_definitions(blocks, definitions=None)
...
function find_callname(func_ir, expr, typemap=None, definition_finder=get_definition)
...
function find_build_sequence(func_ir, var)
...
function find_const(func_ir, var)
...
function _create_function_from_code_obj(fcode, func_env, func_arg, func_clo, glbls)
    #     """
...
function get_ir_of_code(glbls, fcode)
    #     """
...
function replace_arg_nodes(block, args)
    #     """
...
function replace_returns(blocks, target, return_label)
    #     """
...
function gen_np_call(func_as_str, func, lhs, args, typingctx, typemap, calltypes)
    scope = args[0].scope
    loc = args[0].loc
    ...
    # np call: lhs = math.multiply(np_attr(, args))
...
function dump_block(label, block)
        print(label, ":")
    #     for stmt in block.body:
            print("    ", stmt)
...
function is_operator_or_getitem(expr)
...
function set_index_var_of_get_setitem(stmt, new_index)
    #     if is_getitem(stmt):
    #         if stmt.value.op == 'getitem':
...
function is_namedtuple_class(c)
...
function fill_block_with_call(newblock, callee, label_next, inputs, outputs)
...
function fill_callee_prologue(block, inputs, label_next)
    #     """
...
function fill_callee_epilogue(block, outputs)
    #     """
...
function find_outer_value(func_ir, var)
...
function raise_on_unsupported_feature(func_ir, typemap)
    #     """
    ...
    #     """
    gdb_calls = math.divide([] # accumulate calls to gdb, gdb_init)
...
function warn_deprecated(func_ir, typemap)
...
function resolve_func_from_module(func_ir, node)
    #     """
...
function enforce_no_dels(func_ir)
    #     """
...
function enforce_no_phis(func_ir)
    #     """
...
function convert_code_obj_to_function(code_obj, caller_ir)
    #     """
    ...
    nargs = math.subtract(n_allargs, n_kwargs)
...
function fixup_var_define_in_scope(blocks)
...
function transfer_scope(block, scope)
...
function is_setup_with(stmt)
        return isinstance(stmt, ir.EnterWith)


function is_terminator(stmt)
        return isinstance(stmt, ir.Terminator)


function is_raise(stmt)
