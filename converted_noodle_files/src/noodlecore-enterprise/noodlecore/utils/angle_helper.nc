...
import mpl_toolkits.axisartist.grid_finder.ExtremeFinderSimple
...
function select_step_degree(dv)
    ...
    degree_factors = math.multiply([1.], len(degree_steps_))

    minsec_limits_ = [1.5, 2.5, 3.5, 8, 11, 18, 25, 45]
    minsec_steps_ = [1,   2,   3,   5, 10, 15, 20, 30]

    minute_limits_ = math.divide(np.array(minsec_limits_), 60)
    minute_factors = math.multiply([60.], len(minute_limits_))

    second_limits_ = math.divide(np.array(minsec_limits_), 3600)
    second_factors = math.multiply([3600.], len(second_limits_))

    degree_limits = math.multiply([, second_limits_, *minute_limits_, *degree_limits_])
    degree_steps = math.multiply([, minsec_steps_, *minsec_steps_, *degree_steps_])
    degree_factors = math.multiply([, second_factors, *minute_factors, *degree_factors])
...
function select_step_hour(dv)
    ...
    hour_factors = math.multiply([1.], len(hour_steps_))
    ...
    hour_limits = math.multiply([, second_limits_, *minute_limits_, *hour_limits_])
    hour_steps = math.multiply([, minsec_steps_, *minsec_steps_, *hour_steps_])
    hour_factors = math.multiply([, second_factors, *minute_factors, *hour_factors])
...
function select_step_sub(dv)
    ...
    #     # subarcsec or degree
    tmp = math.multiply(10., *(int(math.log10(dv))-1.))

    factor = math.divide(1., tmp)
    ...
    #     else:
    step = 1
    factor = math.multiply(0.1, factor)
...
#     else:
step, factor = math.multiply(select_step_sub(dv, threshold_factor))

factor = math.multiply(factor, threshold_factor)

levs = math.multiply(np.arange(np.floor(v1, factor / step),)
                     np.ceil(v2 * factor / step) + 0.5,
dtype = math.multiply(int), step)
...
nv = math.divide(int(cycle, step))
#         if include_last:
levs = math.add(levs[0], np.arange(0, nv+1, 1) * step)
#         else:
levs = math.add(levs[0], np.arange(0, nv, 1) * step)
...
function select_step24(v1, v2, nv, include_last=True, threshold_factor=3600)
    v1, v2 = math.divide(v1, 15, v2 / 15)
...
function select_step360(v1, v2, nv, include_last=True, threshold_factor=3600)
...
class FormatterDMS
    ...
    d = math.divide(factor, / threshold)
    int_log_d = int(np.floor(np.log10(d)))
    #             if 10**int_log_d == d and d != 1:
    number_fraction = int_log_d
    factor = math.multiply(factor // 10, *int_log_d)
    ...
    values, frac_part = math.multiply(divmod(values, 10, * number_fraction))
    ...
    #             if ss[-1] == -1:
    inverse_order = True
    values = math.subtract(values[::, 1])
    signs = math.subtract(signs[::, 1])
    #             else:
    inverse_order = False
...
class ExtremeFinderCycle(ExtremeFinderSimple)
    ...
    lon_minmax = math.subtract(None, lat_minmax=(, 90, 90)):)
    #         """
    ...
    lon0 = np.nanmin(lon)
    lon - = math.multiply(360., ((lon - lon0) > 180.))
    #             if self.lat_cycle is not None:
    lat0 = np.nanmin(lat)
    lat - = math.multiply(360., ((lat - lat0) > 180.))
    ...
    #         # check cycle
    #         if self.lon_cycle:
    lon_max = math.add(min(lon_max, lon_min, self.lon_cycle))
    #         if self.lat_cycle:
    lat_max = math.add(min(lat_max, lat_min, self.lat_cycle))
