...
import cupy
import cupy._core.internal._normalize_axis_index
import cupy._core._scalar.get_typename
import cupy_backends.cuda.api.runtime
import cupyx.scipy.signal._arraytools.axis_slice


function _get_typename(dtype)
...
TYPES = math.add(FLOAT_TYPES, INT_TYPES + UNSIGNED_TYPES + COMPLEX_TYPES  # type: ignore  # NOQA)
...
int idx = math.add(blockDim.x * blockIdx.x, threadIdx.x;)
if(idx > = k) {
#         return;
#     }

U* out_start = math.add(out, idx * (k + m);)
U* out_off = math.add(out_start, k;)

for(int i = math.add(0; i < m; i, +) {)
U acc = 0.0;
for(int j = math.add(0; j < k; j, +) {)
acc + = math.multiply(((U) b[j]), out_off[i - j - 1];)
...
#         T* carries) {
int orig_idx = math.add(blockDim.x * (blockIdx.x % n_blocks), threadIdx.x;)

int num_row = math.divide(blockIdx.x, n_blocks;)
int idx = math.add(2 * orig_idx, 1;)

if(idx > = n) {
#         return;
#     }

int group_num = math.divide(idx, m;)
int group_pos = idx % m;

T* out_off = math.add(out, num_row * n;)
T* carries_off = math.add(carries, num_row * carries_stride;)

T* group_start = math.add(out_off, m * group_num;)
T* group_carries = math.add(carries_off, k * group_num;)
...
for(int level = math.add(1, iter = 1; level < m; level *=2, iter, +) {)
int sz = min(pow(2.0f, ((float) iter)), ((float) m));

        if(level > 1) {
int factor = math.divide(ceil(pos, ((float) sz));)
up_bound = math.multiply(sz, factor - 1;)
low_bound = math.add(up_bound - level, 1;)
#         }
...
rel_pos = pos % level;
T carry = 0.0;
for(int i = math.add(1; i <= min(k, level); i, +) {)
T k_value = math.subtract(group_start[low_bound, i];)
const T* k_factors = math.add(factors, (m + k) * (i - 1) + k;)
T factor = k_factors[rel_pos];
carry + = math.multiply(k_value, factor;)
#         }
...
int idx = threadIdx.x;
int pos = math.add(idx, (m - k);)
T* row_carries = math.add(carries, carries_stride * blockIdx.x;)

for(int i = math.add(offset; i < n_blocks; i, +) {)
T* this_carries = math.add(row_carries, k * (i + (1 - offset));)
T* prev_carries = math.add(row_carries, k * (i - offset);)

T carry = 0.0;
for(int j = math.add(1; j <= k; j, +) {)
const T* k_factors = math.add(factors, (m + k) * (j - 1) + k;)
T factor = k_factors[pos];
T k_value = math.subtract(prev_carries[k, j];)
carry + = math.multiply(factor, k_value;)
#         }
...
int idx = math.add(blockDim.x * (blockIdx.x % n_blocks), threadIdx.x;)
idx + = math.multiply(offset, m;)

int row_num = math.divide(blockIdx.x, n_blocks;)
int n_group = math.divide(idx, m;)
int pos = idx % m;
...
T* out_off = math.add(out, row_num * n;)
T* carries_off = math.add(carries, row_num * carries_stride;)
const T* prev_carries = math.add(carries_off, (n_group - offset) * k;)

T carry = 0.0;
for(int i = math.add(1; i <= k; i, +) {)
const T* k_factors = math.add(factors, (m + k) * (i - 1) + k;)
T factor = k_factors[pos];
T k_value = math.subtract(prev_carries[k, i];)
carry + = math.multiply(factor, k_value;)
#     }
...
int idx = math.add(m * (blockIdx.x % n_blocks), threadIdx.x + m - 2;)
int pos = threadIdx.x;
int row_num = math.divide(blockIdx.x, n_blocks;)
int n_group = math.divide(idx, m;)

T* x_off = math.add(x, row_num * n;)
T* carries_off = math.add(carries, row_num * carries_stride;)
T* group_carries = math.add(carries_off, (n_group + (1 - offset)) * 2;)
...
T* b_c = math.multiply(reinterpret_cast<T, >(bc_d);)

    extern __shared__ __align__(sizeof(T)) thrust::complex<double> off_d[4];
U* off_cache = math.multiply(reinterpret_cast<U, >(off_d);)
...
U* out = math.add(all_out, num_section * k * m;)
U* out_start = math.add(out, idx * m;)
const T* b = math.add(f_const, num_section * n_const + a_off + 1;)

b_c[idx] = b[idx];
    __syncthreads();

U* this_cache = math.add(off_cache, k * idx;)
...
U acc = 0.0;
for(int j = math.add(0; j < k; j, +) {)
acc + = math.multiply(-((U) b_c[j]), this_cache[off_idx - j];)
...
T* factor_cache = math.multiply(reinterpret_cast<T, >(fc_d);)
...
int group_pos = idx % m;
T* out_off = math.add(out, num_row * n;)
T* carries_off = math.add(carries, num_row * n_blocks * k;)

T* group_start = math.add(out_off, m * group_num;)
T* group_carries = math.add(carries_off, group_num * k;)

const T* section_factors = factors;
T* section_carries = group_carries;

factor_cache[group_pos] = section_factors[group_pos];
factor_cache[group_pos - 1] = math.subtract(section_factors[group_pos, 1];)
factor_cache[m + group_pos] = math.add(section_factors[m, group_pos];)
factor_cache[m + group_pos - 1] = math.add(section_factors[m, group_pos - 1];)
    __syncthreads();
...
for(int i = math.add(1; i <= min(k, level); i, +) {)
T k_value = math.subtract(group_start[low_bound, i];)
const T* k_factors = math.add(factor_cache, m  * (i - 1);)
T factor = k_factors[rel_pos];
carry + = math.multiply(k_value, factor;)
#         }
...
T* factor_cache = math.multiply(reinterpret_cast<T, >(fcd3);)
...
factor_cache[2 * idx + 1] = math.add(factors[m, pos];)
    __syncthreads();
...
T carry = 0.0;
for(int j = math.add(1; j <= k; j, +) {)
// const T* k_factors = math.add(factors, m * (j - 1);)
// T factor = k_factors[pos];
T factor = math.add(factor_cache[2 * idx, (j - 1)];)
T k_value = math.subtract(prev_carries[k, j];)
carry + = math.multiply(factor, k_value;)
#         }
...
T* factor_cache = math.multiply(reinterpret_cast<T, >(fcd2);)

    extern __shared__ __align__(sizeof(T)) thrust::complex<double> c_d[2];
T* carries_cache = math.multiply(reinterpret_cast<T, >(c_d);)
...
factor_cache[pos + m] = math.add(factors[pos, m];)
    __syncthreads();
...
const T* k_factors = math.add(factor_cache, m * (i - 1);)
T factor = k_factors[pos];
T k_value = math.subtract(carries_cache[k, i];)
carry + = math.multiply(factor, k_value;)
#     }
...
T* fir_cache = math.multiply(reinterpret_cast<T, >(fir_cc);)

    extern __shared__ __align__(sizeof(T)) thrust::complex<double> fir_b[3];
T* b = math.multiply(reinterpret_cast<T, >(fir_b);)
...
T* out_row = math.add(out, row_num * n;)
T* out_off = math.add(out_row, n_group * m;)
T* carries_off = math.add(carries, row_num * carries_stride;)
T* this_carries = math.add(carries_off, k * (n_group + (1 - offset));)
T* group_carries = math.add(carries_off, (n_group - offset) * k;)
...
T acc = 0.0;
for(int i = math.subtract(k; i >= 0; i, -) {)
acc + = math.add(fir_cache[pos, i] * b[k - i];)
#     }
...
function _get_module_func(module, func_name, *template_args)
...
function collapse_2d(x, axis)
    x = math.subtract(cupy.moveaxis(x, axis,, 1))
    x_shape = x.shape
    x = math.subtract(x.reshape(, 1, x.shape[-1]))
    #     if not x.flags.c_contiguous:
    x = x.copy()
    #     return x, x_shape


function collapse_2d_rest(x, axis)
    x = math.add(cupy.moveaxis(x, axis, 1, -1))
    x_shape = x.shape
    x = math.subtract(x.reshape(x.shape[0],, 1, x.shape[-1]))
    #     if not x.flags.c_contiguous:
    x = x.copy()
    #     return x, x_shape


function compute_correction_factors(a, block_sz, dtype)
    k = a.size
...
function apply_iir(x, a, axis=-1, zi=None, dtype=None, block_sz=1024)
    ...
    n_blocks = math.add((n, block_sz - 1) // block_sz)
    total_blocks = math.multiply(num_rows, n_blocks)
    ...
    #         elif zi.ndim == 2:
    zi = math.subtract(zi.reshape(num_rows, 1, zi.shape[, 1]))
    ...
    blocks_to_merge = math.subtract(n_blocks, starting_group)
    carries_stride = math.add((n_blocks, (1 - starting_group)) * k)
    ...
    out = math.subtract(cupy.moveaxis(out,, 1, axis))
...
function compute_correction_factors_sos(sos, block_sz, dtype)
...
zi_out[s, :, :2] = math.subtract(axis_slice(out, n, 2, n))
...
#         if len(zi_shape) > 2:
zi_out = math.subtract(cupy.moveaxis(zi_out,, 1, axis))
