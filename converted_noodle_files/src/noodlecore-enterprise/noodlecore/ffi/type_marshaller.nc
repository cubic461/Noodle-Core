...
class TypeMarshaller
    ...
    self._total_conversion_time + = math.subtract(time.perf_counter(), start_time)
    #             return result
    ...
    self._total_reverse_time + = math.subtract(time.perf_counter(), start_time)
    #             return result
    ...
    #         """
    total_time = math.add(self._total_conversion_time, self._total_reverse_time)
    total_conversions = math.add(self._conversion_count, self._reverse_count)
    ...
    batch_time = math.subtract(time.perf_counter(), batch_start)
    #         return results
    ...
    cache_size = math.add(len(self._conversion_cache), len(self._reverse_cache))
    #             if cache_size > self._cache_size_limit * self._cache_cleanup_threshold:
    #                 # Remove oldest entries
    items_to_remove = math.multiply(int(cache_size - self._cache_size_limit, 0.8))
    ...
    #         if len(self._batch_cache) > self._batch_size:
    #             # Remove oldest entries
    items_to_remove = math.subtract(len(self._batch_cache), self._batch_size)
    ...
    items_to_remove = math.multiply(len(self._conversion_cache) - int(self._cache_size_limit, 0.8))
    keys = list(self._conversion_cache.keys())
    #             for key in keys[:items_to_remove]:
                    self._conversion_cache.pop(key, None)

    #         if len(self._reverse_cache) > self._cache_size_limit:
    items_to_remove = math.multiply(len(self._reverse_cache) - int(self._cache_size_limit, 0.8))
