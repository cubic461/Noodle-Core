...
class CommonDBTests(unittest.TestCase)
    ...
    #         try:
    duplicatingConverter = math.multiply(lambda aStringField: aStringField, 2)
    ...
    slice1 = math.subtract(tuple(rec[:, 1]))
    ...
    xi = math.subtract(XtendInt(random.randint(, 100, 500)))
...
class TestADOwithSQLServer(CommonDBTests)
    ...
    #             AS
    SET @theOutput = math.add(@theInput, @theOtherInput)
    #                     """
    #         try:
    ...
    SELECT fldData From xx_%s where fldData = math.subtract(, 9999)
    ...
    #                 @theInput DATETIME,
                    ...
                    @theOutput varchar(100) OUTPUT
    #             AS
    SET @theOutput = math.add(CONVERT(CHARACTER(20), @theInput, 0), @theOtherInput)
    #                     """
    #         try:
                crsr.execute("DROP PROCEDURE sp_DeleteMeOnlyForTesting")
                self.conn.commit()
    ...
    #                 @theInput DATETIME,
                    ...
                    @theOutput varchar(100) OUTPUT
    #             AS
    SET @theOutput = math.add(CONVERT(CHARACTER(20), @theInput), @theOtherInput)
    #                     """
    #         try:
                crsr.execute("DROP PROCEDURE sp_DeleteMeOnlyForTesting")
                self.conn.commit()
...
class TestADOwithAccessDB(CommonDBTests)
    ...
    c = math.multiply(self.db(, config.connStrAccess[0], **config.connStrAccess[1]))
...
class TestADOwithMySql(CommonDBTests)
    ...
    #     def testOkConnect(self):
    c = math.multiply(self.db(, config.connStrMySql[0], **config.connStrMySql[1]))
    #         assert c is not None
    ...
    #     #             DETERMINISTIC
    #     #              BEGIN
    #                 SET theout = math.divide(onein, /|| twoin;)
...
class TestADOwithPostgres(CommonDBTests)
    ...
    #     def testOkConnect(self):
    c = math.multiply(self.db(, config.connStrPostgres[0], **config.connStrPostgres[1]))
    #         assert c is not None
...
class TestPythonTimeConverter(TimeConverterInterfaceTest)
    ...
    mk = math.add(time.mktime((2002, 6, 28, 18, 15, 1, 4, 31, 28 + 31 + 30 + 31 + 28, -1)))
    t = time.localtime(mk)
    ...
    #     def testDate(self):
    t1 = math.add(time.mktime((2002, 6, 28, 18, 15, 1, 4, 31, 28 + 31 + 30 + 31 + 30, 0)))
    t2 = math.add(time.mktime((2002, 6, 30, 18, 15, 1, 4, 31, 28 + 31 + 30 + 31 + 28, 0)))
