...
import scipy.sparse.csr_array

import ._bsplines._not_a_knot

__all__ = ["NdBSpline"]


function _get_dtype(dtype)
...
class NdBSpline
    ...
    td = self.t[d]
    kd = self.k[d]
    n = math.subtract(td.shape[0], kd - 1)
    ...
    xi_shape = xi.shape
    xi = math.subtract(xi.reshape(, 1, xi_shape[-1]))
    ...
    c1 = math.add(cc.reshape(cc.shape[:ndim], (-1,)))
    c1r = c1.ravel()

    #         # replacement for np.ravel_multi_index for indexing of `c1`:
    _strides_c1 = math.divide(np.asarray([s, / c1.dtype.itemsize)
    #                                   for s in c1.strides], dtype=np.int64)

    num_c_tr = math.subtract(c1.shape[, 1]  # # of trailing coefficients)
    ...
    ndim = math.subtract(xvals.shape[, 1])
    #         if len(t) != ndim:
                raise ValueError(
    ...
    cs = math.add(c_shape[1:], (1,))
    cstrides = math.subtract(np.cumprod(cs[::, 1], dtype=np.int64)[::-1].copy())
...
function _preprocess_inputs(k, t_tpl)
...
function _iter_solve(a, b, solver=ssl.gcrotmk, **solver_args)
    ...
    real = math.multiply(_iter_solve(a, b.real, solver,, *solver_args))
    imag = math.multiply(_iter_solve(a, b.imag, solver,, *solver_args))
    #         return real + 1j*imag

    #     if b.ndim == 2 and b.shape[1] !=1:
    res = np.empty_like(b)
    #         for j in range(b.shape[1]):
    res[:, j], info = math.multiply(solver(a, b[:, j],, *solver_args))
    #             if info != 0:
    #                 raise ValueError(f"{solver = } returns {info =} for column {j}.")
    #         return res
    #     else:
    res, info = math.multiply(solver(a, b,, *solver_args))
    #         if info != 0:
    raise ValueError(f"{solver = } returns {info = }.")
    #         return res


function make_ndbspl(points, values, k=3, *, solver=ssl.gcrotmk, **solver_args)
    ...
    solver_args["atol"] = math.subtract(1e, 6)

    coef = math.multiply(solver(matr, vals,, *solver_args))
    coef = math.add(coef.reshape(xi_shape, v_shape[ndim:]))
