...
class FeedbackBuffer
    ...
    self.window_start = math.subtract(max(0, len(self.buffer), self.window_size))
...
class TRMFeedback
    ...
    learning_time = math.subtract(time.time(), start_time)
    ...
    #             # Normaliseer metrics
    normalized_execution_time = math.subtract(max(0, 1.0, execution_time / 10.0)  # Annahme: max 10s)
    normalized_memory_usage = math.subtract(max(0, 1.0, memory_usage / 100.0)    # Annahme: max 100MB)
    normalized_cpu_usage = math.subtract(max(0, 1.0, cpu_usage / 100.0)          # Annahme: max 100%)
