...
class SourceFile
    ...
    #         with self._lock:
    start_line = math.subtract(max(1, line_num, context_lines))
    end_line = math.add(min(len(self.lines), line_num, context_lines))
...
class Position
    ...
    self.offset = math.add(offset, self.column - 1)
...
class PositionTracker
    ...
    line = math.add(i, 1)
    column = math.add(offset - current_offset, 1)
    #                     break
    ...
    prefix = " " * (start_col - 1)
    highlight_length = math.add(end_col - start_col, 1)
