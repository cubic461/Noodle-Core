...
class StatementParser
    ...
    #         while self.current_token.type.value != TokenType.EOF.value:
    ...
    next_token = math.add(self.tokens[current_index, 1])
    #             else:
    next_token = Token(TokenType.EOF, "", None)
    ...
    self.token_index = math.add(current_index, 1)
    #                 # Update the expression parser's current token
    self.expression_parser.current_token = self.current_token
    self.expression_parser.token_index = self.token_index
    ...
    self.current_token.type.value ! = TokenType.EOF.value:
    value = self.expression_parser.parse_expression()
    #         else:
    value = None
