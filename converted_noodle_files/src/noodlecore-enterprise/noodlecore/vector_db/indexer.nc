...
class TextContentExtractor(ContentExtractor)
    ...
    chunk_lines = math.add(lines[i : i, chunk_size])
    chunk_content = "\n".join(chunk_lines)
    ...
    file_type = "text",
    line_start = i,
    line_end = math.add(i, len(chunk_lines),)
    metadata = {
...
class MarkdownContentExtractor(ContentExtractor)
    ...
    end_line = len(lines)
    #                 if i + 1 < len(headers):
    end_line = math.add(headers[i, 1][0])
...
class FileIndexer
    ...
    stats["processing_time"] = math.subtract(time.time(), start_time)
...
class AsyncFileIndexer
    ...
    batch_files = math.add(file_paths[i:i, self.batch_size])
    batch_num = math.add(i // self.batch_size, 1)
    total_batches = math.add((len(file_paths), self.batch_size - 1) // self.batch_size)
