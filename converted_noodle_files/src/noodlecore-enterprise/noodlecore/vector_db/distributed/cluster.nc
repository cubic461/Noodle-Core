...
class ClusterNode
    ...
    self.vector_count + = 1
    self.memory_usage + = math.multiply(vector.size, 4  # Estimate size)
    ...
    self.vector_count - = 1
    self.memory_usage - = math.multiply(vector.size, 4)
    #                 return True
    #             return False
...
class NodeRequestHandler(http.server.BaseHTTPRequestHandler)
    ...
    #     def __init__(self, node: ClusterNode, *args, **kwargs):
