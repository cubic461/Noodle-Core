...
function scan_directory(path, recursive=True, max_depth=3)
...
function search_files_in_workspace(query, directory=None)
...
function search_content_in_files(query, directory=None)
...
function ide_execution_improve()
...
function handle_join_session(data)
...
function handle_code_change(data)
    session_id = data.get('session_id')
    code = data.get('code')
...
function main()
    ...
    #         if os.path.exists(WORKSPACE_DIR):
                logger.info(f"✅ Workspace directory accessible: {WORKSPACE_DIR}")
    ...
    #     except Exception as e:
            logger.error(f"❌ Cannot access workspace directory: {e}")
