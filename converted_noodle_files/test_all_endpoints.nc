...
function test_endpoint(name, url, method="GET", data=None)
...
function main()
        print("🚀 NoodleCore Enhanced IDE - API Connection Test")
    print(" = " * 60)
    ...
    #     if success:
    passed_tests + = 1
            print(f"   📁 Files: {len(data.get('files', []))} found")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   ⚡ Execution: {data.get('execution_time')}s")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   📊 Analysis: Performance {int(data.get('performance_score', 0)*100)}%")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   💡 Suggestions: {len(data.get('suggestions', []))} found")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   🔍 Review Score: {data.get('overall_score')}/100")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   🚀 Learning ID: {data.get('learning_id', 'N/A')}")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   📄 Matches: {data.get('total', 0)}")
    ...
    #     if success:
    passed_tests + = 1
            print(f"   🤖 Models: {data.get('models_deployed', 0)} deployed")
    ...
    #     if passed_tests == total_tests:
            print("🎉 ALL TESTS PASSED! The NoodleCore Enhanced IDE is fully functional!")
            print("\n✅ All frontend buttons are now connected to real backend functionality:")
            print("   • AI Code Analysis → /api/v1/ai/analyze")
            print("   • AI Suggestions → /api/v1/ai/suggest")
            print("   • AI Code Review → /api/v1/ai/review")
            print("   • Learning System → /api/v1/learning/status & /api/v1/learning/trigger")
            print("   • File Search → /api/v1/search/files & /api/v1/search/content")
            print("   • Code Execution → /api/v1/execution/run & /api/v1/execution/improve")
            print("   • Collaboration → WebSocket connections")
            print("   • AI Deployment → /api/v1/ai/deployment/status & /api/v1/ai/deployment/deploy")

            print("\n🚀 Ready to use! Open http://localhost:8080/enhanced-ide-fixed.html")
    #         return True
    #     else:
            print(f"❌ {total_tests - passed_tests} tests failed. Check server logs.")
    #         return False
