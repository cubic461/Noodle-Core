...
try
    ...
    AI_INFRASTRUCTURE_AVAILABLE = True
        print("OK AI Infrastructure imported successfully")
except ImportError as e
        print(f"❌ AI Infrastructure not available: {e}")
    AI_INFRASTRUCTURE_AVAILABLE = False
    #     # Add detailed diagnostic logging
    #     import traceback
        print(f"❌ AI Infrastructure import traceback: {traceback.format_exc()}")

# Import TRM configuration classes
try
    #     # Fix import path - use absolute import from noodlecore.src
    #     import sys
    #     import os
    #     # Add noodle-core/src to sys.path if not already there
    noodle_core_path = os.path.join(os.path.dirname(__file__), 'src')
    #     if noodle_core_path not in sys.path:
            sys.path.insert(0, noodle_core_path)

    #     from noodlecore.trm.trm_agent_base import TRMModelConfig, OptimizationConfig, FeedbackConfig
    TRM_CONFIG_AVAILABLE = True
        print("OK TRM Configuration classes imported successfully")
except ImportError as e
        print(f"❌ TRM Configuration not available: {e}")
    TRM_CONFIG_AVAILABLE = False
    #     # Add detailed diagnostic logging
    #     import traceback
        print(f"❌ TRM Configuration import traceback: {traceback.format_exc()}")
...
class TRMAgent
    ...
    #         # Initialize TRM configuration classes with error handling
    #         if TRM_CONFIG_AVAILABLE:
    #             try:
    self.model_config = TRMModelConfig()
    self.optimization_config = OptimizationConfig()
    self.feedback_config = FeedbackConfig()
                    print("✅ TRM-Agent: Configuration classes initialized successfully")
    #             except Exception as e:
                    print(f"TRM-Agent: Failed to initialize configuration classes: {e}")
    self.model_config = None
    self.optimization_config = None
    self.feedback_config = None
    #         else:
    self.model_config = None
    self.optimization_config = None
    self.feedback_config = None
    ...
    #         """Setup AI client with real Z.ai integration."""
    #         try:
    ...
    #                 print("TRM-Agent: AI client initialized with real Z.ai")
    #             else:
                    print("TRM-Agent: No provider configs found, using fallback")
    self.ai_client = None
    ...
    #             if os.getenv("NOODLE_ZAI_API_KEY"):
    #                 return {
    #                     "zai": {
    #                         "provider": "zai",
                            "model": os.getenv("NOODLE_AI_MODEL", "glm-4.6"),
    #                         "base_url": "https://open.bigmodel.cn/api/paas/v4",
    #                         "auth_env": "NOODLE_ZAI_API_KEY"
    #                     }
    #                 }
...
#     async def analyze_code_with_ai(self, file_path: str, code_content: str) -> Dict[str, Any]:
...
ast_analysis = self._analyze_code_ast(file_path, code_content)
            ...
            # Then, get AI-powered suggestions (now async)
ai_suggestions = await self._get_ai_optimization_suggestions(file_path, code_content, ast_analysis)
...
#     async def _get_ai_optimization_suggestions(self, file_path: str, code_content: str, ast_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
...
#                 }
                ...
                # Call AI through the existing client (async)
response = await self.ai_client.invoke(
...
#     def _learn_from_analysis(self, analysis_result: Dict[str, Any]) -> None:
...
#             if "complexity_vs_optimization" not in self.performance_patterns:
self.performance_patterns["complexity_vs_optimization"] = []
