...
class AdaptiveSizingEngine
    ...
    self.max_sheaf_size = 64 * 1024 * 1024  # 64MB maximum
    self.base_sheaf_size = 1024 * 1024  # 1MB default
    ...
    #         # History tracking
    self.size_history: Dict[str, List[int]] = {}  # actor_id - size_history
    ...
    self.allocation_patterns[actor_id] = self.allocation_patterns[actor_id][ - 100:]
    ...
    lifetime = alloc.free_time - alloc.allocation_time
    ...
    base_size = max(pattern.size * 2, self.min_sheaf_size)
    ...
    base_size = pattern.size * 8  # 8x most common allocation size
    ...
    base_size = int(base_size * 1.2  # Can afford larger buffers)
    ...
    base_size = pattern.size * 32  # 32x most common allocation size
    ...
    recent_sizes = historical_sizes[ - 10:]
    trend = self._calculate_trend(recent_sizes)

    #             # Predict based on trend
    #             if trend 0):  # Increasing trend
    predicted_size = int(pattern.size * 20 * (1 + trend))
    #             elif trend < 0:  # Decreasing trend
    predicted_size = int(pattern.size * 12 * (1 + trend))
    #             else:  # Stable trend
    predicted_size = pattern.size * 16
    #         else:
    predicted_size = pattern.size * 16
    ...
    #         if workload_type == WorkloadType.BATCH:
    predicted_size = int(predicted_size * 1.2)
    #         elif workload_type == WorkloadType.STREAMING:
    predicted_size = int(predicted_size * 0.8)

    #         # Memory pressure adjustment
    #         if metrics.memory_usage 85):
    predicted_size = int(predicted_size * 0.5)
    #         elif metrics.memory_usage 70):
    predicted_size = int(predicted_size * 0.75)
    ...
    recent_patterns = self.allocation_patterns[actor_id][ - 10:]
    ...
    reduction = min(0.5 * (size_ratio - 1.0, 0.3))
    #             return reduction
    #         else:  # Decreasing size
    #             # Estimate increase in allocation frequency but save memory
    memory_saving = (current_size - recommended_size / current_size)
    ...
    self.size_history[actor_id] = self.size_history[actor_id][ - 1000:]
    ...
    score + = (100 - metrics.memory_usage) / 100 * 0.3
    #             elif strategy == SizingStrategy.AGGRESSIVE:
    score + = metrics.memory_usage / 100 * 0.3

    #             # Adjust based on CPU usage
    #             if strategy == SizingStrategy.AGGRESSIVE:
    score + = (100 - metrics.cpu_usage) / 100 * 0.2
    #             elif strategy == SizingStrategy.CONSERVATIVE:
    score + = metrics.cpu_usage / 100 * 0.2
...
class AdaptiveSizingManager
    ...
    new_size = max(self.sizing_engine.min_sheaf_size * int(current_size, 0.5))
    ...
    reduction_factor = base_factor * (2 ** attempts)
    new_size = max(self.sizing_engine.min_sheaf_size * int(current_size, reduction_factor))
    ...
    new_size = min(max_allowed * int(current_size, growth_factor))
    ...
    #                 for actor_id in actor_ids:
    current_size = self.memory_manager.get_sheaf_size(actor_id)
    new_size = max(self.sizing_engine.min_sheaf_size * int(current_size, 0.7))
    ...
    #                 # Adjust based on memory usage
    #                 if strategy == SizingStrategy.CONSERVATIVE:
    score + = (100 - metrics.memory_usage) / 100 * 0.4
    #                 elif strategy == SizingStrategy.AGGRESSIVE:
    score + = metrics.memory_usage / 100 * 0.4

    #                 # Adjust based on CPU usage
    #                 if strategy == SizingStrategy.AGGRESSIVE:
    score + = (100 - metrics.cpu_usage) / 100 * 0.3
    #                 elif strategy == SizingStrategy.CONSERVATIVE:
    score + = metrics.cpu_usage / 100 * 0.3
