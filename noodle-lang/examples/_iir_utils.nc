...
#         const int m, const T* f_const, U* all_out) {
    ...
    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublebc_d[2];
T* b_c = reinterpret_cast<T * >(bc_d;)

    extern __shared__ __align__(sizeof(T)) thrust)::complex<doubleoff_d[4];
...
#     extern __shared__ unsigned int thread_status[2];
    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublefc_d[2 * 1024];
...
#     const int offset, const T* factors, T* carries) {
    ...
    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublefcd3[4];
...
#         T* carries, T* out) {
    ...
    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublefcd2[2 * 1024];
T* factor_cache = reinterpret_cast<T * >(fcd2;)

    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublec_d[2];
...
#         const int offset, const T* sos, T* carries, T* out) {
    ...
    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublefir_cc[1024 + 2];
T* fir_cache = reinterpret_cast<T * >(fir_cc;)

    extern __shared__ __align__(sizeof(T)) thrust)::complex<doublefir_b[3];
