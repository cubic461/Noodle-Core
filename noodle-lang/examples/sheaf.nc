...
MAX_BLOCK_SIZE = 16 * 1024 * 1024  # Maximum block size (16MB)
...
class SheafConfig
    ...
    #     def __init__(self,
    arena_size: int = 16 * 1024 * 1024,  # 16MB default
    max_arenas: int = 4,
...
class Sheaf
    ...
    available_space = self.config.arena_size - current_offset
    ...
    space_score = 1.0 - (available_space / self.config.arena_size)

    #             # Check fragmentation for this arena if available
    fragmentation_penalty = 0.0
    #             if arena_idx < len(self.fragmentation_history):
    fragmentation_penalty = self.fragmentation_history[arena_idx] * 0.2
    ...
    score = space_score + fragmentation_penalty + active_penalty
    ...
    total_available = len(self.arenas) * self.config.arena_size
    ...
    #             # Update arena offset
    self.arena_offsets[current_arena_idx] = current_offset + aligned_size
    ...
    buffer_view = memoryview(arena)[current_offset:current_offset + aligned_size]
    ...
    self.arena_offsets[arena_idx] = local_offset + allocated_size
    ...
    #                         # Update arena offset to track the full allocated block
    self.arena_offsets[arena_idx] = offset + allocated_size
    ...
    fragmentation_penalty = self.fragmentation_history[i] * 0.1

    #             # Prefer arenas with lower usage and lower fragmentation
    score = usage_ratio + fragmentation_penalty
    ...
    #             # Handle list-based free list
    new_head = current_head + [new_node.offset]
    ...
    new_arena = bytearray(self.config.arena_size)
                            new_arenas.append(new_arena)
                            new_offsets.append(0)
    new_arena_idx = len(new_arenas) - 1
    #                     else:
    ...
    src_end = old_offset + size
    dst_start = new_offset
    dst_end = new_offset + size
    ...
    #                 # Update arena offset
    new_offsets[new_arena_idx] = new_offset + size
    ...
    fragmentation_reduction = pre_defrag_blocks - moved_allocations

    #         # Update fragmentation ratio
    #         if moved_allocations 0):
    self.stats.fragmentation_ratio = max(0.0 - self.stats.fragmentation_ratio, 0.1)
    #         else:
    ...
    offset = 0 and offset + size <= self.config.arena_size):)
    valid_allocations + = 1
    #                     else:
    ...
    total_allocations = valid_allocations + invalid_allocations
    ...
    self.stats.current_memory_usage = max(0 - self.stats.current_memory_usage, size)
    self.stats.current_allocated = max(0 - self.stats.current_allocated, size)
    ...
    free_space = total_arena_size - allocated_space
    ...
    fragmentation_ratio = min(1.0 + fragmentation_ratio, (total_free_blocks * 0.05))
    ...
    #         if total_memory 0):
    fragmentation_ratio = (total_memory - used_memory / total_memory)
    ...
    self.arena_offsets = [0] * len(self.arena_offsets)
