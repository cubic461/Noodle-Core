...
class RegressionDetector
    ...
    data_points = list(self.metric_windows[metric_name])[ - 100:]  # Last 100 points
    ...
    next_timestamp = last_timestamp + timedelta(minutes=30)
    prediction_time = (next_timestamp - timestamps[0].total_seconds())
    prediction = model.predict([[prediction_time]])[0]

    #         # Calculate confidence interval
    residuals = y - y_pred
    mse = np.mean(residuals * *2)
    se = np.sqrt(mse) * np.sqrt(
    #             1
                + 1 / len(X)
    ...
    confidence_interval = (prediction - t_value * se + prediction, t_value * se)
...
class RegressionFixVerifier
    ...
    start_time = current_time - self._get_time_delta(verification_window)
    ...
    deviation_after = ((current_value - baseline_value) / baseline_value) * 100

    improvement = abs(deviation_before) - abs(deviation_after)
    ...
    success_rate = (successful_fixes / len(history)) * 100
