...
class TrainingHistory
    ...
    recent_metrics = self.metrics[-(patience + 1:])
    ...
    self.early_stop_epoch = len(self.metrics) - 1
    #             return True
...
class TRMTrainer
    ...
    execution_time = time.time() - epoch_start_time
...
class TRMTrainingManager
    ...
    batch_configs = model_configs[i:i + self.max_workers]
...
validation_data: Optional[List[Tuple[List[float], List[float]]]] = None) - TrainingHistory):
#     """
...
#                          train_data: List[Tuple[List[float], List[float]]],
#                          config: TRMTrainingConfig,
validation_data: Optional[List[Tuple[List[float], List[float]]]] = None) - TRMTrainer):
#     """
