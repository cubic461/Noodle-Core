...
class MemoryLeakDetector
    ...
    is_increasing = all(memory_trend[i] <= memory_trend[i + 1]
    ...
    #         # Age factor
    age = time.time() - candidate.creation_time
    ...
    #         # Access pattern
    time_since_access = time.time() - candidate.last_access_time
    ...
    leak_candidates = self.leak_candidates[ - 50:]  # Last 50 candidates
    #             )
    ...
    self.leak_candidates = self.leak_candidates[ - 500:]
...
if __name__ == "__main__"
    ...
    gc = LeakDetectionGarbageCollector(max_heap_size=1024 * 1024 * 1024)
    ...
    objects = objects[: - 10]  # First 90 objects properly managed
