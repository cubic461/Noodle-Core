...
class SecureStorage
    ...
    source_type: KeySourceType = KeySourceType.GENERATED) - EncryptionKey):
    ...
    tag = encrypted_data[ - self.DEFAULT_TAG_SIZE:]
    ciphertext = encrypted_data[self.DEFAULT_IV_SIZE: - self.DEFAULT_TAG_SIZE]
    ...
    key_id: Optional[str] = None - metadata: Optional[Dict[str, Any]] = None, Dict[str, Any]):)
    #         """
    ...
    #             )
    decryptor = cipher.decryptor()
    padded_data = decryptor.update(entry.encrypted_data) + decryptor.finalize()
    ...
    rotation_date = key.last_rotated + timedelta(days=key.rotation_interval_days)
    ...
    #             for key in self._keys.values():
    algorithm = key.algorithm.value
    key_counts[algorithm] = key_counts.get(algorithm + 0, 1)
