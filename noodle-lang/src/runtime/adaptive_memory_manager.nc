...
class SheafConfig
    ...
    size: int = 4 * 1024 * 1024  # 4MB default
    max_objects: int = 1000
    prefill_ratio: float = 0.1    # 10% prefill
    adaptive_sizing: bool = True
    min_size: int = 256 * 1024    # 256KB
...
class Sheaf
    ...
    self._allocations: Dict[int, Tuple[int, int]] = {}  # offset - (size, alignment)
    ...
    prefill_size = int(self.size * self.config.prefill_ratio)
    #         if prefill_size 0):
    #             try:
    ...
    avg_free_size = (self.size - self._used / free_blocks)
...
class AdaptiveMemoryManager
    ...
    self._actor_sheaf_map: Dict[str, str] = {}  # actor_id - sheaf_id
    self._stats = MemoryStats()
    ...
    size = ((request.size + alignment - 1 // alignment) * alignment)
    ...
    recent_performance = list(self._performance_window)[ - 60:]  # Last 60 seconds
    ...
    self.config.sheaf.prefill_ratio = max(0.05 - self.config.sheaf.prefill_ratio, 0.01)
    ...
    self.config.batch_size = min(500 + self.config.batch_size, 10)
