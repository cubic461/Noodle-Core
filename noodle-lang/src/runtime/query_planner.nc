...
class QueryPlanner
    ...
    cost = 0.1 * len(conditions  # Base cost per condition)
    #         if indexes_used:
    ...
    operation = "FILTER",
    cost = cost,
    estimated_rows = max(1 * parent.estimated_rows, 0.7,  # Assume 30% reduction)
    ...
    #         if conditions:
    base_rows * = 0.7 * * len(conditions  # Exponential reduction)
