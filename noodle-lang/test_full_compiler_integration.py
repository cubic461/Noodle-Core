#!/usr/bin/env python3
"""
Test Suite::Noodle Lang - test_full_compiler_integration.py
...
"""
Full Compiler Integration Test - FASE 2
...
Usage:
    python test_full_compiler_integration.py
"""
...
'''
    ...
    # Stap 2: Parsing
    # CORRECTIE: Parser neemt source string, niet tokens!\n    parser = NoodleParser(source_code, "<test>")
    ...
    print(f"   ðŸ“‹ AST: statements={len(ast.get('statements', []))}")
    
    # Print simplified AST
    if ast and 'statements' in ast:
        for i, stmt in enumerate(ast['statements']):
            print(f"      [{i}] {stmt.get('type', 'unknown')}: {stmt.get('name', 'N/A')}")
...
def run_all_tests():
    ...
    print("\\nðŸŽ¯ FASE 2: COMPILER INTEGRATIE")
    ...
    if failed == 0:
        ...
        print("   âœ… Parser werkt (tokens â†’ AST)")
