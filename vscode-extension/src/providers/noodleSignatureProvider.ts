 ...
 * in NoodleCore files.
 */
export class NoodleSignatureProvider implements vscode.SignatureHelpProvider {
     ...
     */
    private matchFunctionCall(text: string): { name: string; params: string[] } | undefined {
     ...
     */
    private matchAIOperation(text: string): { name: string; params: string[] } | undefined {
     ...
     */
    private getFunctionSignature(match: { name: string; params: string[] }): vscode.SignatureHelp {
        ...
        const defaultSignature = new vscode.SignatureInformation(
            `${match.name}(...)`,
            `Function ${match.name}`
        );
     ...
     */
    private getAIOperationSignature(match: { name: string; params: string[] }): vscode.SignatureHelp {
     ...
     */
    private getFunctionSignatures(): vscode.SignatureInformation[] {
        return [
            // Basic functions
     ...
     */
    private getAIOperationSignatures(): vscode.SignatureInformation[] {
        return [
            // AI model operations
            new vscode.SignatureInformation(
                'train(dataset, epochs?, batchSize?, validationData?)',
                'Train AI model'
            ),
            new vscode.SignatureInformation(
                'predict(input, batchSize?)',
                'Make predictions with AI model'
            ),
            new vscode.SignatureInformation(
                'validate(testData, metrics?)',
                'Validate AI model'
            ),
            new vscode.SignatureInformation(
            ...
            new vscode.SignatureInformation(
                'fit(dataset, epochs?, callbacks?)',
                'Fit AI model to data'
            ),
            new vscode.SignatureInformation(
            ...
            // AI agent operations
            new vscode.SignatureInformation(
                'learn(experience, reward?)',
                'Learn from experience'
            ),
            new vscode.SignatureInformation(
                'act(state, explorationRate?)',
                'Take action based on state'
            ),
            new vscode.SignatureInformation(
                'observe(state, action, reward, nextState)',
            ...
            new vscode.SignatureInformation(
                'run(inputData, config?)',
                'Run AI pipeline'
            ),
            new vscode.SignatureInformation(
                'addStep(name, function, config?)',
            ...
            new vscode.SignatureInformation(
                'load(filepath, format?, preprocessing?)',
                'Load dataset'
            ),
            new vscode.SignatureInformation(
                'split(ratio, shuffle?)',
            ...
            new vscode.SignatureInformation(
                'transform(data, type, config?)',
                'Transform data using AI'
            ),
            new vscode.SignatureInformation(
                'optimize(hyperparameters, objective?)',
                'Optimize AI model'
            ),
            new vscode.SignatureInformation(
                'refactor(code, type, config?)',
