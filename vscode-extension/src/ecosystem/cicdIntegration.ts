...
export type CICDProvider = 'github-actions' | 'gitlab-ci' | 'jenkins' | 'azure-devops' | 'circleci' | 'travis-ci' | 'bitbucket-pipelines';
export type CICDStatus = 'pending' | 'running' | 'success' | 'failure' | 'cancelled' | 'skipped';
export type CICDTrigger = 'push' | 'pull-request' | 'schedule' | 'manual' | 'webhook';
...
export class CICDIntegration {
    ...
    constructor(workspaceRoot: string, githubIntegration?: GitHubIntegration) {
    ...
    private detectProvider(): void {
        const configFiles = [
            { path: '.github/workflows', provider: 'github-actions' as CICDProvider },
            { path: '.gitlab-ci.yml', provider: 'gitlab-ci' as CICDProvider },
            { path: 'Jenkinsfile', provider: 'jenkins' as CICDProvider },
            { path: 'azure-pipelines.yml', provider: 'azure-devops' as CICDProvider },
            { path: '.circleci/config.yml', provider: 'circleci' as CICDProvider },
            { path: '.travis.yml', provider: 'travis-ci' as CICDProvider },
            { path: 'bitbucket-pipelines.yml', provider: 'bitbucket-pipelines' as CICDProvider }
        ];
    ...
    private getProviderIcon(): string {
        const icons: { [key in CICDProvider]: string } = {
     ...
     */
    private async getGitHubActionsPipelines(): Promise<CICDPipeline[]> {
     ...
     */
    private async getGitLabCIPipelines(): Promise<CICDPipeline[]> {
        try {
     ...
     */
    private async getJenkinsPipelines(): Promise<CICDPipeline[]> {
        try {
     ...
     */
    private async getAzureDevOpsPipelines(): Promise<CICDPipeline[]> {
        try {
     ...
     */
    private async getGenericPipelines(): Promise<CICDPipeline[]> {
        return [{
            id: 'generic',
            name: 'Generic CI/CD',
     ...
     */
    private mapGitHubStatus(status: string): CICDStatus {
        const statusMap: { [key: string]: CICDStatus } = {
            'queued': 'pending',
            'in_progress': 'running',
            'completed': 'success',
            'failure': 'failure',
     ...
     */
    public async triggerPipeline(pipelineId: string, inputs?: { [key: string]: any }): Promise<void> {
        try {
            switch (this.provider) {
                case 'github-actions':
                    if (this.githubIntegration) {
     ...
     */
    private async triggerGitLabCI(): Promise<void> {
     ...
     */
    private async triggerJenkins(): Promise<void> {
     ...
     * Get pipeline runs
     */
    public async getPipelineRuns(pipelineId: string): Promise<CICDRun[]> {
        try {
            switch (this.provider) {
                case 'github-actions':
     ...
     */
    private async getGitHubActionsRuns(pipelineId: string): Promise<CICDRun[]> {
        if (!this.githubIntegration) {
            throw new Error('GitHub integration not available');
        }
     ...
     */
    public async createConfiguration(provider: CICDProvider): Promise<void> {
        try {
            ...
            vscode.window.showInformationMessage(
                `${provider} configuration created at ${configPath}`
            );
...
`;
     ...
     */
    private getGitLabCITemplate(): string {
        return `stages:
  - test
  - build
  - deploy
...
`;
     ...
     */
    private getJenkinsTemplate(): string {
        return `pipeline {
    agent any
...
`;
     ...
     */
    private getAzureDevOpsTemplate(): string {
        return `trigger:
  branches:
    include:
    - main
...
`;
     ...
     */
    private formatPipelinesForDisplay(pipelines: CICDPipeline[]): string {
