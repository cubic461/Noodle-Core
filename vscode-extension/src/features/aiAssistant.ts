 ...
 */
export class NoodleAIAssistant {
    ...
    private registerCommands(): void {
        ...
        const refactorCommand = vscode.commands.registerCommand(
            'noodle.aiAssistant.refactor',
            () => this.refactorSelection()
        );
        this.disposables.push(refactorCommand);

        // Explain current selection
        const explainCommand = vscode.commands.registerCommand(
            'noodle.aiAssistant.explain',
     ...
     */
    private async getAIAssistantWebviewContent(panel: vscode.WebviewPanel): Promise<string> {
...
<body>
     ...
     */
    private analyzeForIssues(code: string): Array<{
        type: string;
        message: string;
        line: number;
        code: string;
    ...
    private async analyzeCode(code: string): Promise<void> {
        try {
            ...
            this.outputChannel.appendLine((error as Error).message || 'Unknown error');
        }
    }
     ...
     */
    private async refactorCode(code: string, type: string): Promise<void> {
        try {
     ...
     * Generate code from prompt
     */
    private async generateCodeFromPrompt(prompt: string): Promise<void> {
        try {
     ...
     */
    private async fixCodeErrors(code: string): Promise<void> {
        try {
     ...
     */
    private async optimizeCodePerformance(code: string): Promise<void> {
        try {
     ...
     */
    private async performCodeReview(code: string): Promise<void> {
        try {
     ...
     * Call NoodleCore AI service
     */
    private async callAIService(action: string, data: any): Promise<any> {
