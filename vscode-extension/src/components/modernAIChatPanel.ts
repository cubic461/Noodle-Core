...
export class ModernAIChatPanel {
    private _panel: vscode.WebviewPanel | undefined;
    private _disposables: vscode.Disposable[] = [];
    private _state: AIChatState;
    private _backendService: NoodleBackendService;
    private _context: vscode.ExtensionContext;

    constructor(
        extensionUri: vscode.Uri,
        backendService: NoodleBackendService,
    ...
    private async _getBackendResponse(content: string): Promise<{
        content: string;
        thinking?: string;
        suggestions?: string[];
    }> {
        try {
            const response = await this._backendService.makeAIChatRequest(
                content,
                this._state.currentProvider,
                this._state.currentModel,
                this._state.messages.slice(-10), // Last 10 messages for context
                this._state.context
            );
    ...
    private async _getFallbackResponse(content: string): Promise<string> {
...
For now, you can use the other Noodle IDE features like code analysis and project exploration.`;
    ...
    private async _applySuggestion(suggestion: string, type: string) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
    ...
    private async _explainCode(code: string) {
        try {
    ...
    private async _fixCode(code: string) {
        try {
    ...
    private async _optimizeCode(code: string) {
        try {
    ...
    private async _generateCode(prompt: string) {
        try {
    ...
    private _setProvider(provider: string, model: string) {
