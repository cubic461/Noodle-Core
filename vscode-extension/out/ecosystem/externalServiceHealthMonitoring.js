...
exports.ExternalServiceHealthMonitoring = void 0;
const vscode = __importStar(require("vscode"));
const axios = __importStar(require("axios"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class ExternalServiceHealthMonitoring {
    constructor(context) {
    ...
    startMonitoring() {
        ...
        const interval = config.get('monitoringInterval', 30000); // 30 seconds default
     ...
     */
    async checkThresholds(serviceId) {
        ...
        for (const threshold of serviceThresholds) {
            ...
            if (triggered) {
                await this.generateAlert(serviceId, threshold.severity, `Threshold exceeded: ${threshold.metric} ${threshold.operator} ${threshold.value} (current: ${currentValue})`, threshold);
            }
        }
    }
    /**
     ...
     */
    async generateAlert(serviceId, type, message, threshold) {
        const service = this.services.get(serviceId);
        if (!service) {
            return;
        }
    ...
    async sendAlertNotification(alert) {
        ...
        const channels = config.get('channels', ['console']);
     ...
     */
    recordMetric(serviceId, metric, value) {
        const healthMetric = {
...
exports.ExternalServiceHealthMonitoring = ExternalServiceHealthMonitoring;
//# sourceMappingURL=externalServiceHealthMonitoring.js.map