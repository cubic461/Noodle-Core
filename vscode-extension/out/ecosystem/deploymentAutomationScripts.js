...
exports.DeploymentAutomationScripts = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const yaml = __importStar(require("js-yaml"));
const child_process = __importStar(require("child_process"));
class DeploymentAutomationScripts {
    constructor(context) {
     ...
     */
    setKubernetesIntegration(kubernetesIntegration) {
     ...
     */
    async createDeployment(environmentId, templateId, customSteps, variables) {
        try {
     ...
     * Build deployment steps
     */
    buildDeploymentSteps(environment, template, customSteps, variables) {
        const steps = [];
     ...
     * Execute command
     */
    async executeCommand(command, timeout, environment) {
    ...
    async rollbackDeployment(deploymentId) {
        ...
        try {
            const result = await this.executeCommand(environment.config.rollback.rollbackCommand, 300000, environment.variables);
            if (result.success) {
     ...
     */
    getDeploymentsForEnvironment(environmentId) {
        return Array.from(this.deployments.values())
     ...
     */
    async deployToKubernetes(manifestPath, namespace, environmentId) {
        try {
            ...
            let deploymentId;
            if (environmentId) {
                deploymentId = await this.createDeployment(environmentId, undefined, [{
     ...
     */
    async scaleKubernetesDeployment(deploymentName, replicas, environmentId) {
        try {
            if (!this.kubernetesIntegration) {
                throw new Error('Kubernetes integration not available');
            }
     ...
     */
    async getKubernetesPodLogs(podName, follow, tail) {
        try {
            if (!this.kubernetesIntegration) {
                throw new Error('Kubernetes integration not available');
            }
     ...
     */
    async deleteKubernetesResource(resourceType, resourceName, environmentId) {
        try {
            if (!this.kubernetesIntegration) {
                throw new Error('Kubernetes integration not available');
            }
     ...
     */
    async setupKubernetesPortForward(podName, localPort, remotePort, environmentId) {
        try {
            if (!this.kubernetesIntegration) {
                throw new Error('Kubernetes integration not available');
            }
     ...
     */
    async createKubernetesManifest(resourceType, environmentId) {
        try {
            if (!this.kubernetesIntegration) {
                throw new Error('Kubernetes integration not available');
            }
     ...
     */
    async createKubernetesEnvironment(name, namespace, kubeconfig) {
        try {
            const environment = {
     ...
     */
    getKubernetesEnvironments() {
        return this.getEnvironments().filter(env => env.type === 'container');
    }
    /**
     * Check if Kubernetes integration is available
     */
    isKubernetesIntegrationAvailable() {
        return this.kubernetesIntegration !== undefined;
    }
}
exports.DeploymentAutomationScripts = DeploymentAutomationScripts;
//# sourceMappingURL=deploymentAutomationScripts.js.map