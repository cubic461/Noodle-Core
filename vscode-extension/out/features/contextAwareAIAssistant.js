...
exports.ContextAwareAIAssistant = void 0;
...
class ContextAwareAIAssistant {
    constructor(context) {
        ...
        this.statusBarItem = vscode.window.createStatusBarItem('noodle.contextAwareAI', vscode.StatusBarAlignment.Right, 105);
    ...
    registerCommands() {
        ...
        const showCommand = vscode.commands.registerCommand('noodle.contextAwareAI.show', () => this.showContextAwareAI());
        this.disposables.push(showCommand);
        // Analyze current context
        const analyzeContextCommand = vscode.commands.registerCommand('noodle.contextAwareAI.analyzeContext', () => this.analyzeCurrentContext());
        this.disposables.push(analyzeContextCommand);
        // Toggle auto-switch
        const toggleAutoSwitchCommand = vscode.commands.registerCommand('noodle.contextAwareAI.toggleAutoSwitch', () => this.toggleAutoSwitch());
        this.disposables.push(toggleAutoSwitchCommand);
        // Force role switch
        const forceSwitchCommand = vscode.commands.registerCommand('noodle.contextAwareAI.forceSwitch', () => this.forceRoleSwitch());
        this.disposables.push(forceSwitchCommand);
        // Update memory context
        const updateMemoryCommand = vscode.commands.registerCommand('noodle.contextAwareAI.updateMemory', () => this.updateMemoryContext());
        this.disposables.push(updateMemoryCommand);
        // Clear memory context
        const clearMemoryCommand = vscode.commands.registerCommand('noodle.contextAwareAI.clearMemory', () => this.clearMemoryContext());
     ...
     */
    async getDocumentSymbols(editor) {
        if (!editor)
            return [];
        try {
            return await vscode.commands.executeCommand('vscode.executeDocumentSymbolProvider', editor.document.uri);
        }
        catch (error) {
    ...
    async getRecentFiles() {
        try {
            const history = await vscode.commands.executeCommand('vscode.openRecent');
            return history || [];
        }
        catch (error) {
     ...
     */
    async checkForAutoRoleSwitch(oldContext, newContext) {
        ...
        if (success) {
            ...
            vscode.window.showInformationMessage(`Auto-switched to ${newRole?.name} role: ${topRule.description}`, 'View Details', 'Undo').then(selection => {
    ...
    getApplicableRules(context) {
        return this.contextSwitchRules.filter(rule => {
            ...
            if (conditions.diagnostics) {
                const hasMatchingDiagnostics = context.diagnostics.some(diag => conditions.diagnostics.includes(diag.severity));
                if (!hasMatchingDiagnostics)
                    return false;
            }
            // Check git status
            if (conditions.gitStatus && context.gitStatus) {
                const hasMatchingStatus = conditions.gitStatus.some(status => context.gitStatus.files.some((file) => file.status === status));
     ...
     * Show context-aware AI assistant
     */
    async showContextAwareAI() {
        const panel = vscode.window.createWebviewPanel('noodleContextAwareAI', 'Noodle Context-Aware AI Assistant', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    showContextAnalysis() {
        ...
        const panel = vscode.window.createWebviewPanel('noodleContextAnalysis', 'Context Analysis', vscode.ViewColumn.Two, {
            enableScripts: true,
     ...
     * Clear memory context
     */
    async clearMemoryContext() {
        const confirmation = await vscode.window.showWarningMessage('Clear all memory context? This will remove all learned preferences and patterns.', 'Clear', 'Cancel');
     ...
     */
    async getContextAwareAIWebviewContent(panel) {
...
<body>
     ...
     */
    getContextAnalysisWebviewContent(analysis) {
        const nonce = this.getNonce();
        const cspSource = [
            `default-src 'none';`,
            `script-src 'nonce-${nonce}' 'unsafe-inline';`,
...
exports.ContextAwareAIAssistant = ContextAwareAIAssistant;
//# sourceMappingURL=contextAwareAIAssistant.js.map