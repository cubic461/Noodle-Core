#!/usr/bin/env python3
"""
Noodle Core::Ultimate Syntax Fix - ultimate_syntax_fix.py
...
"""Ultimate fix for all syntax errors in native_gui_ide.py"""
...
# Fix all syntax errors with a complete rewrite of problematic sections
...
content = re.sub(
    r'self\.terminal_output\.insert\(\'end\', f"\[WARNING\] Dependencies: No requirements file found\\n"\)\s*self\.terminal_output\.insert\(\'end\', f"\[WARNING\] Dependencies: No requirements file found\\n"\)\s*self\.terminal_output\.insert\(\'end\', f"\[WARNING\] Dependencies: No requirements file found\\n"\)\s*self\.terminal_output\.insert\(\'end\', f"\[HEALTH\] Overall Health Score: \{health_score\}/100\\n"\)',
    'self.terminal_output.insert(\'end\', f"[HEALTH] Overall Health Score: {health_score}/100\\n")',
    content
)

# 6. Fix messagebox functions with proper string handling
content = re.sub(
    r'def fix_noodlecore_syntax\(self\):.*?messagebox\.showinfo\(\s*"Syntax Fixer",\s*("NoodleCore Syntax Fixer is not available.*?")\s*\)',
...
content = re.sub(
    r'def show_python_integration_settings\(self\):.*?messagebox\.showinfo\(\s*"Python Integration",\s*("Python Integration settings are not available.*?")\s*\)',
...
print("Applied ultimate syntax fixes to native_gui_ide.py")
