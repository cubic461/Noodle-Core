...
func main():
    println("🔍 Debugging Encrypted Configuration System...")
    println("=" * 60)
    ...
    println(f"Expected config directory: {expected_config_dir}")
    
    # Check if directory exists
    if expected_config_dir.exists():
        println(f"✅ Config directory exists")
        # List files in directory
        files = list(expected_config_dir.iterdir())
        println(f"Files in config directory: {files}")
        ...
        println(f"Key file exists: {key_file.exists()}")
        ...
        if ide_dir.exists():
            ...
            println(f"Files in IDE directory: {ide_files}")
            ...
            if ide_settings.exists():
                ...
                with open(ide_settings, 'r') as f:
                    ...
                    println(f"IDE settings content: {json.dumps(settings, indent=2)}")
    else:
        println(f"❌ Config directory does not exist")
    
    # Test # importing the encrypted config module
    println(f"\n" + "=" * 60)
    println("Testing encrypted config module # import...")
    try:
        # from noodlecore.config # import EncryptedConfigManager, get_ai_config, save_ai_config
        println("✅ Successfully # imported encrypted configuration module")
        
        # Test creating a config manager
        println("\nTesting config manager creation...")
        config_manager = EncryptedConfigManager()
        println(f"Config manager directory: {config_manager.config_dir}")
        println(f"Encrypted config file path: {config_manager.encrypted_config_file}")
        println(f"Key file path: {config_manager.key_file}")
        
        # Test getting AI config
        println("\nTesting get_ai_config...")
        ai_config = get_ai_config()
        println(f"Provider: {ai_config.get('provider', 'Not set')}")
        println(f"Model: {ai_config.get('model', 'Not set')}")
        println(f"API Key set: {'Yes' if ai_config.get('api_key') else 'No'}")
        
        # Test saving AI config
        println("\nTesting save_ai_config...")
        save_ai_config('TestProvider', 'test-model', 'test-api-key-12345', False)
        println("✅ Test config saved")
        
        # Verify save
        println("\nVerifying saved config...")
        updated_config = get_ai_config()
        println(f"Provider: {updated_config.get('provider', 'Not set')}")
        println(f"Model: {updated_config.get('model', 'Not set')}")
        println(f"API Key set: {'Yes' if updated_config.get('api_key') else 'No'}")
        
        # Check if files were actually created
        println(f"\nChecking if files were created...")
        println(f"Encrypted config file exists: {encrypted_config_file.exists()}")
        println(f"Key file exists: {key_file.exists()}")
        
        if encrypted_config_file.exists():
            println(f"Encrypted config file size: {encrypted_config_file.stat().st_size} bytes")
        
        if key_file.exists():
            println(f"Key file size: {key_file.stat().st_size} bytes")
