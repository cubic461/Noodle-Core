#!/usr/bin/env python3
"""
Test Suite::Noodle Core - test_learning_endpoints.py
...
class LearningEndpointTester:
    ...
    def test_all_learning_endpoints(self):
        ...
        print("ðŸ§ª Testing NoodleCore Learning System Endpoints")
        print("=" * 60)
        
        # Test 1: Learning Status
        print("ðŸ“Š Testing Learning Status...")
        ...
        if result["success"]:
            ...
            if "learning_system" in result["response"].get("data", {}):
                print(f"   ðŸ“ˆ Learning System Status: {result['response']['data']['learning_system']}")
        else:
            print(f"âŒ Learning Status: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        ...
        if result["success"]:
            ...
            print(f"âŒ Learning Configuration: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        
        # Test 3: Learning History
        print("\nðŸ“š Testing Learning History...")
        ...
        if result["success"]:
            ...
            if "total_records" in result["response"].get("data", {}):
                print(f"   ðŸ“Š Total Records: {result['response']['data']['total_records']}")
        else:
            print(f"âŒ Learning History: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        ...
        if result["success"]:
            ...
            if "total_models" in result["response"].get("data", {}):
                print(f"   ðŸ¤– Total Models: {result['response']['data']['total_models']}")
        else:
            print(f"âŒ Model Management: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        
        # Test 5: Learning Analytics
        print("\nðŸ“ˆ Testing Learning Analytics...")
        result = self.test_endpoint("GET", f"{API_PREFIX}/learning/analytics",
                                   params={"period": "daily"})
        if result["success"]:
            print("âœ… Learning Analytics: OK")
        else:
            print(f"âŒ Learning Analytics: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        
        # Test 6: Manual Learning Trigger (POST)
        print("\nðŸŽ¯ Testing Manual Learning Trigger...")
        trigger_data = {
        ...
        if result["success"]:
            ...
            if "session_id" in result["response"].get("data", {}):
                print(f"   ðŸŽ¯ Session ID: {result['response']['data']['session_id']}")
        else:
            print(f"âŒ Manual Learning Trigger: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        ...
        if result["success"]:
            ...
            print(f"âŒ Learning Pause: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        ...
        if result["success"]:
            ...
            print(f"âŒ Learning Resume: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        
        # Test 9: Capability Trigger Control
        print("\nðŸŽ›ï¸ Testing Capability Trigger...")
        capability_data = {
            "capability_name": "code_analysis",
            "action": "enable"
        }
        ...
        if result["success"]:
            ...
            if "message" in result["response"].get("data", {}):
                print(f"   ðŸŽ›ï¸ Message: {result['response']['data']['message']}")
        else:
            print(f"âŒ Capability Trigger: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
        
        # Test 10: Learning Feedback
        print("\nðŸ’¬ Testing Learning Feedback...")
        feedback_data = {
            "capability_name": "code_analysis",
        ...
        if result["success"]:
            ...
            if "feedback_id" in result["response"].get("data", {}):
                print(f"   ðŸ’¬ Feedback ID: {result['response']['data']['feedback_id']}")
        else:
            print(f"âŒ Learning Feedback: Failed ({result.get('error', 'HTTP ' + str(result['status_code']))})")
    ...
    def print_summary(self):
        """Print test summary."""
        print("\n" + "=" * 60)
        print("ðŸ“‹ LEARNING SYSTEM TEST SUMMARY")
        print("=" * 60)
        ...
        if failed_tests > 0:
            ...
            for result in self.results:
                if not result["success"]:
                    print(f"   âŒ {result['method']} {result['endpoint']}")
                    if "error" in result:
...
def main():
    """Main test function."""
    print("ðŸš€ NoodleCore Learning System API Test Suite")
    ...
    print(f"\nðŸ’¾ Test results saved to: learning_system_test_results.json")
