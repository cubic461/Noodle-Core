#!/usr/bin/env python3
"""
Test Suite::Noodle Core - test_auto_linter_documentation_integration.py
...
"""
Test script for Auto Linter and Documentation Integration
...
def test_file_processing():
    """Test file processing with linting and documentation."""
    ...
    try:
        ...
        py_file = test_dir / "test_file.py"
        if py_file.exists():
            print(f"âœ“ Processing Python file: {py_file}")
            integration._check_file_for_linting_and_documentation(str(py_file))
        
        # Test JavaScript file
        js_file = test_dir / "test_file.js"
        if js_file.exists():
            print(f"âœ“ Processing JavaScript file: {js_file}")
            integration._check_file_for_linting_and_documentation(str(js_file))
        
        # Test NoodleCore file
        nc_file = test_dir / "test_file.nc"
        if nc_file.exists():
            print(f"âœ“ Processing NoodleCore file: {nc_file}")
            integration._check_file_for_linting_and_documentation(str(nc_file))
...
def test_self_improvement_integration():
    """Test the self-improvement integration with auto linter/documentation."""
    print("Testing Self-Improvement Integration...")
    ...
    try:
        ...
        # Check if auto linter/documentation integration is initialized
        if si_integration.auto_linter_doc_integration:
            print("âœ“ Auto Linter/Documentation integration initialized in Self-Improvement")
        else:
            print("âœ— Auto Linter/Documentation integration not initialized in Self-Improvement")
            return False
        ...
        # Set current project path for testing
        si_integration.ide_instance = type('MockIDE', (), {
            'current_project_path': str(test_dir)
        })()
        
        # Test file checking
        py_file = test_dir / "test_file.py"
        if py_file.exists():
            print(f"âœ“ Checking file for improvements: {py_file}")
            si_integration._check_file_for_improvements(str(py_file))
        
        # Test auto linter/documentation check
        si_integration.auto_linter_doc_integration._check_file_for_linting_and_documentation(str(py_file))
        
        # Get statistics
        stats = si_integration.get_improvement_status()
        print(f"âœ“ Self-Improvement status: {stats}")
        ...
        print(f"âœ— Error testing self-improvement integration: {e}")
        import traceback
        traceback.print_exc()
        return False
