#!/usr/bin/env python3
"""
Test Suite::Noodle Core - test_ide_complete_integration.py
...
noodle_function main():
    ...
    def test_execution_system_integration(self) -> None:
        ...
        print("\n" + "="*70)
        print("âš¡ Testing Script Execution System")
        print("="*70)
        ...
        if result.success:
            print(f"âœ… Script Improvement: OK ({response_time:.2f}ms)")
        else:
            print(f"âŒ Script Improvement: FAILED ({response_time:.2f}ms)")
    ...
    def test_performance_requirements(self) -> None:
        ...
        if max_response_time <= PERFORMANCE_THRESHOLD_MS:
            print(f"âœ… Performance Threshold: PASS (<{PERFORMANCE_THRESHOLD_MS}ms)")
        else:
    ...
    def test_security_and_compliance(self) -> None:
        ...
        if result.status_code == 404 and '<script>' not in str(response_data):
            print(f"âœ… Error Sanitization: PASS ({response_time:.2f}ms)")
        else:
            print(f"âŒ Error Sanitization: FAIL ({response_time:.2f}ms)")
    ...
    def test_end_to_end_workflows(self) -> None:
        ...
        print("   Testing Search â†’ Execution pipeline...")
        workflow_start = time.time()
        ...
        print(f"âœ… Searchâ†’Execution Pipeline: Complete ({workflow_time:.2f}ms)")
