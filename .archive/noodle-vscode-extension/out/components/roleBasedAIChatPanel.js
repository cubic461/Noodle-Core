...
exports.RoleBasedAIChatPanel = void 0;
const vscode = __importStar(require("vscode"));
const roleBasedAIService_1 = require("../services/roleBasedAIService");
class RoleBasedAIChatPanel {
    constructor(_extensionUri) {
        this._extensionUri = _extensionUri;
        this._disposables = [];
        this._roleService = new roleBasedAIService_1.RoleBasedAIService();
        this._state = {
            messages: [],
            currentRole: null,
            availableRoles: [],
    ...
    show() {
        ...
        this._panel = vscode.window.createWebviewPanel('noodleRoleBasedAIChat', 'Noodle AI Role Assistant', vscode.ViewColumn.One, {
            enableScripts: true,
    ...
    async _handleSendMessage(content) {
        if (!content.trim() || !this._state.currentRole)
            return;
        const userMessage = {
            id: this._generateId(),
            role: 'user',
     ...
     */
    async _handleSwitchRole(roleId) {
     ...
     */
    async _sendRoleDocument(roleId) {
     ...
     */
    async _handleAnalyzeCode(code) {
        if (!this._state.currentRole)
            return;
        this._state.isTyping = true;
        this._sendState();
        try {
     ...
     */
    async _handleGenerateCode(prompt) {
        if (!this._state.currentRole)
            return;
        this._state.isTyping = true;
        this._sendState();
     ...
     */
    async _handleRefactorCode(code, refactorType) {
        if (!this._state.currentRole)
            return;
        this._state.isTyping = true;
        this._sendState();
     ...
     * Handle code fixing
     */
    async _handleFixCode(code, errors) {
        if (!this._state.currentRole)
            return;
        this._state.isTyping = true;
        this._sendState();
...
exports.RoleBasedAIChatPanel = RoleBasedAIChatPanel;
//# sourceMappingURL=roleBasedAIChatPanel.js.map