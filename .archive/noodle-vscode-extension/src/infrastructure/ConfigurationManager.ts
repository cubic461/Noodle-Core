...
export interface ConfigurationSection {
    [key: string]: ConfigurationSchema | ConfigurationSection;
}
...
export class ConfigurationManager extends EventEmitter {
    ...
    private schema: ConfigurationSection = {};
     ...
     */
    public get<T>(key: string, scope?: vscode.ConfigurationScope): T | undefined {
        try {
            const value = this.configuration.get<T>(key, undefined, scope);
     ...
     */
    public async set(key: string, value: any, target?: vscode.ConfigurationTarget): Promise<boolean> {
        try {
            // Validate value
     ...
     */
    public watchConfigurationFile(filePath: string): void {
        try {
     ...
     * Load configuration schema
     */
    private async loadConfigurationSchema(): Promise<void> {
        try {
            // Default schema
            this.schema = {
                ecosystem: {
     ...
     * Validate schema
     */
    private validateSchema(schema: ConfigurationSection, config: any, path: string, errors: string[]): void {
        for (const [key, schemaValue] of Object.entries(schema)) {
            ...
            if (typeof schemaValue === 'object' && schemaValue.type) {
                // Validate property
                const propertySchema = schemaValue as ConfigurationSchema;
                ...
                if (value !== undefined) {
                    this.validateSchema(schemaValue as ConfigurationSection, value, currentPath, errors);
                }
            }
        }
    }
     ...
     * Get schema for key
     */
    private getSchemaForKey(key: string): ConfigurationSchema | undefined {
        try {
            ...
            return current as ConfigurationSchema;
        } catch (error) {
            return undefined;
        }
    }
