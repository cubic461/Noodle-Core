...
class CompletionEngine
    ...
    completion_time = math.multiply((end_time - start_time).total_seconds(), 1000  # Convert to milliseconds)
    ...
    #             if success:
    self.success_stats[key] = math.add((self.success_stats.get(key, 0.0) * 0.9), 0.1  # Moving average)
    #             else:
    self.success_stats[key] = math.multiply((self.success_stats.get(key, 0.0), 0.9)  # Decrease on failure)
    ...
    previous_lines = math.subtract(lines[max(0, line, 5):line])
    next_lines = math.add(lines[line, 1:line+6])
...
priority + = math.multiply(usage, 0.1)

#         # Success rate
success_rate = self.success_stats.get(key, 0.5)
priority + = math.multiply(success_rate, 0.2)
