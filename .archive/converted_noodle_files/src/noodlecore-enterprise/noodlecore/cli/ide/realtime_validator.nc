...
class ValidationResult
    ...
    self.end_character = math.add(end_character or character, len(message))
    self.code = code
    self.source = source
...
class RealtimeValidator
    ...
    validation_time = math.multiply((end_time - start_time).total_seconds(), 1000  # Convert to milliseconds)
    ...
    char_pos = math.subtract(match.start(), line_start)
    ...
    character = char_pos,
    end_line = line_num,
    end_character = math.add(char_pos, len(match.group()),)
    code = 'SEC001',
    source = 'security',
    ...
    code_lines = math.subtract(non_empty_lines, comment_lines)
    ...
    complexity_score = math.add(func_count, var_count + (loop_count * 2) + (if_count * 1.5))

                # Calculate maintainability index (simplified)
    maintainability_index = math.multiply(max(0, 100 - complexity_score, 2))
