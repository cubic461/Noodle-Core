...
class TokenType(Enum)
    #     """Token types for the Noodle language"""
...
class Token
    ...
    return self.type ! = TokenType.ERROR and self.value is not None
    ...
    #         else:
                return Token(TokenType.ERROR, f"Unknown operator: {operator}", position)
    ...
    #         else:
                return Token(TokenType.ERROR, f"Unknown delimiter: {delimiter}", position)
...
class TokenStream
    ...
    #         with self._lock:
    peek_pos = math.add(self.position, offset - 1)
...
class TokenValidator
    ...
    prev_token = math.subtract(tokens[i, 1])
...
class TokenCache
    ...
    #         with self._lock:
    total_requests = math.add(self.hit_count, self.miss_count)
