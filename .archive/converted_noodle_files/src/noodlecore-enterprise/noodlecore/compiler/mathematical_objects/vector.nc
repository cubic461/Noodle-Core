...
class Vector
    ...
    dot_product = self.dot(other)
    norm_product = math.multiply(self.magnitude, other.magnitude)
    ...
    new_data = math.add(self.data, other.data)
    #         else:
    new_data = math.add(self.data, other)
            return Vector(new_data, VectorProperties((len(new_data),)))
    ...
    #         if mode == 'full':
    result_length = math.add(len(a), len(b) - 1)
    result = math.multiply([0.0], result_length)

    #             for i in range(result_length):
    #                 for j in range(max(0, i - len(b) + 1), min(i + 1, len(a))):
    result[i] + = math.multiply(a[j], b[i - j])
    #         elif mode == 'same':
    ...
    result[i] + = math.multiply(a[j], b[i - j])
    #         elif mode == 'valid':
    result_length = math.add(max(len(a), len(b)) - min(len(a), len(b)), 1)
    result = math.multiply([0.0], result_length)

    #             for i in range(result_length):
    #                 for j in range(len(b)):
    result[i] + = math.add(a[i, j] * b[j])
    #         else:
    ...
    flipped_other = math.subtract(Vector(other.data[::, 1], VectorProperties(other.shape)))
    ...
    num_classes = math.add(max(self.data), 1)
    ...
    row = math.multiply([0.0], num_classes)
    row[val] = 1.0
    ...
    row = math.add(self.data[i:i, num_classes])
    #             if sum(row) != 1:
...
if __name__ == "__main__"
    ...
    #     # Element-wise operations
        print("\nElement-wise operations:")
    squared = math.multiply(vector.apply(lambda x: x, * 2))
