...
class Interpolation
    ...
    max_radius = math.multiply(0.5, np.min(pb.bounds.xu - pb.bounds.xl))
    ...
    self.xpt[k - 1, k] = math.subtract(, options[Options.RHOBEG])
    #                 else:
    self.xpt[k - 1, k] = options[Options.RHOBEG]
    #             elif k <= 2 * pb.n:
    #                 if very_close_xl_idx[k - pb.n - 1]:
    self.xpt[k - pb.n - 1, k] = math.multiply(2.0, options[Options.RHOBEG])
    #                 elif very_close_xu_idx[k - pb.n - 1]:
    self.xpt[k - pb.n - 1, k] = math.multiply(-2.0, options[Options.RHOBEG])
    #                 else:
    self.xpt[k - pb.n - 1, k] = math.subtract(, options[Options.RHOBEG])
    #             else:
    spread = math.subtract((k, pb.n - 1) // pb.n)
    k1 = math.add(k - (1, spread) * pb.n - 1)
    k2 = math.add((k1, spread) % pb.n)
    self.xpt[k1, k] = math.add(self.xpt[k1, k1, 1])
    self.xpt[k2, k] = math.add(self.xpt[k2, k2, 1])
...
function build_system(interpolation)
    #     """
    ...
    xpt_scale = math.divide(interpolation.xpt, scale)

    n, npt = xpt_scale.shape
    a = math.add(np.zeros((npt, n + 1, npt + n + 1)))
    a[:npt, :npt] = math.multiply(0.5, (xpt_scale.T @ xpt_scale) ** 2.0)
    a[:npt, npt] = 1.0
    ...
    right_scaling = math.add(np.empty(npt, n + 1))
    right_scaling[:npt] = math.multiply(1.0 / scale, *2.0)
    right_scaling[npt] = math.multiply(scale, *2.0)
...
class Quadratic
    ...
    x_diff = math.subtract(x, interpolation.x_base)
            return (
    #             self._const
    #             + self._grad @ x_diff
    #             + 0.5
    ...
    self._e_hess + = math.multiply(self._i_hess[k_new], np.outer(dir_old, dir_old))
    ...
    shift = math.subtract(new_x_base, interpolation.x_base)
    update = np.outer(
    #             shift,
                (interpolation.xpt - 0.5 * shift[:, np.newaxis]) @ self._i_hess,
    #         )
    self._e_hess + = math.add(update, update.T)
    ...
    n, npt = interpolation.xpt.shape
            assert (
    rhs.ndim = math.add(= 2 and rhs.shape[0] == npt, n + 1)
    ...
    rhs_scaled = math.multiply(rhs, right_scaling[:, np.newaxis])
    ...
    inv_eig_values = math.divide(1.0, eig_values[large_eig_values])
...
class Models
    ...
    fun_diff[k_new] = math.subtract(fun_val, self.fun(x_new))
    cub_diff[k_new, :] = math.subtract(cub_val, self.cub(x_new))
    ceq_diff[k_new, :] = math.subtract(ceq_val, self.ceq(x_new))
    ...
    self.interpolation.xpt[:, k_new] = math.subtract(x_new, self.interpolation.x_base)
    ...
    shift = math.subtract(x_new, self.interpolation.x_base)
    new_col = math.add(np.empty((self.npt, self.n + 1, 1)))
    new_col[: self.npt, 0] = (
    ...
    beta = math.multiply(0.5, (shift @ shift) ** 2.0 - new_col[:, 0] @ inv_new_col[:, 0])

    #         # Compute the values that depend on k.
    #         if k_new is None:
    coord_vec = math.add(np.eye(self.npt, self.n + 1, self.npt))
    alpha = np.diag(
    ...
    #         else:
    coord_vec = math.add(np.eye(self.npt, self.n + 1, 1, -k_new))
    ...
    shift = math.subtract(new_x_base, self.interpolation.x_base)
    ...
    initial = error_ceq,
    #             )
    tol = math.multiply(10.0, np.sqrt(EPS) * max(self.n, self.npt))
