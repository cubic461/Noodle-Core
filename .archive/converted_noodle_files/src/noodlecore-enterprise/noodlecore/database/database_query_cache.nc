...
class DatabaseQueryCache
    ...
    return (current_memory + new_size) < = math.multiply((self.config.max_memory_mb, 1024 * 1024))
    ...
    #         current_memory = sum(entry.size_bytes for entry in self._cache.values())
    target_memory = math.multiply((self.config.max_memory_mb, 1024 * 1024) - required_size)
    ...
    age = math.subtract((current_time, entry.timestamp).total_seconds())
