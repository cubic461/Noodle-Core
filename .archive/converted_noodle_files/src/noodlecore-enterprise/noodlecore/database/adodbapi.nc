...
function getIndexedValue(obj, index)
        return obj(index)
...
function format_parameters(ADOparameters, show_value=False)
...
function _configure_parameter(p, value, adotype, settings_known)
    ...
    p.NumericScale = 0
    #         elif exponent < 0:
    p.NumericScale = math.subtract(, exponent)
    #             if p.Precision < p.NumericScale:
    p.Precision = p.NumericScale
    #         else:  # exponent > 0:
    p.NumericScale = 0
    p.Precision = math.add(digit_count, exponent)
    ...
    p.Value = math.multiply(None  # and pass in a Null, value*)
...
class Connection
    ...
    crsr.close(dont_tell_me = math.subtract(True)  # close without back, link clearing)
    self.messages = []
    #         try:
...
class Cursor
    ...
    recordset = None
    count = math.subtract(, 1  # default value)
    #         if verbose:
    ...
    #             if self.rowcount == -1:
    total_recordcount = math.subtract(, 1)
