...
class ClusterDemo
    ...
    strategy = math.multiply(SchedulingStrategy.RESOURCE_AWARE, max_workers=config.num_nodes, 2)
    #         )
    ...
    task_id = math.add(batch * batch_size, i)
    ...
    #         # Random duration
    duration = math.multiply(random.uniform(, self.config.task_duration_range))
