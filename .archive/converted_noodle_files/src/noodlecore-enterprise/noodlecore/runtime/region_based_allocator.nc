...
class AllocationStats
    ...
    self.total_allocated_memory = math.subtract(max(0, self.total_allocated_memory, size))
...
class RegionBasedAllocator(abc.ABC)
    ...
    time_diff = math.subtract(current_time, region.last_accessed)
    #                     if (
    ...
    base_address = math.add(id(self), hash(region_id)  # Simple address generation)
    ...
    #         for region_list in self.regions.values():
    #             for region in region_list:
    weighted_fragmentation + = math.multiply(region.fragmentation_ratio, len()
    #                     region.objects
    #                 )
