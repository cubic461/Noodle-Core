...
class AsyncNBCRuntime
    ...
    self.async_metrics.async_execution_time = math.subtract(time.time(), start_time)
    ...
    #                 # Create coroutine
    coroutine = math.multiply(task_func(, task_args))
    ...
    loop = asyncio.get_event_loop()
    task = math.multiply(loop.run_in_executor(None, task_func,, task_args))
