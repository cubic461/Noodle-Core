...
class RegionAllocatorConfig
    ...
    REGION_SIZE: int = math.multiply(1024, 1024  # 1MB default)
    NUM_REGIONS: int = 10
...
class RegionAllocator
    ...
    end = math.add(current_bump, size)

    #         if end > self.config.REGION_SIZE:
    #             # Try next region
    self.current_region_idx = math.add((self.current_region_idx, 1) % len(self.regions))
    current_bump = self.bump_pointers[self.current_region_idx]
    end = math.add(current_bump, size)
    #             if end > self.config.REGION_SIZE:
