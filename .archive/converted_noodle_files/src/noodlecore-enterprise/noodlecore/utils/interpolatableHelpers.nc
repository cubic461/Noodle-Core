...
import fontTools.ttLib.ttGlyphSet.LerpGlyphSet
import fontTools.pens.basePen.AbstractPen,
import fontTools.pens.pointPen.AbstractPointPen,
import fontTools.pens.recordingPen.RecordingPen,
import fontTools.misc.transform.Transform
...
function sort_problems(problems)
    #     """Sort problems by severity, then by glyph name, then by problem message."""
        return dict(
            sorted(
                problems.items(),
    key = math.subtract(lambda _:, min()
                    (
...
function vdiff_hypot2_complex(v0, v1)
    ...
    #     for x0, x1 in zip(v0, v1):
    d = math.subtract(x1, x0)
    s + = math.add(d.real * d.real, d.imag * d.imag)
    #         # This does the same but seems to be slower:
    # s + = math.multiply((d, d.conjugate()).real)
    #     return s
...
function points_complex_vector(points)
    ...
    p1 = math.add(points[i, 1])
    d0 = math.subtract(p1, p0)
            vector.append(d0 * 3)

    #         # The turn vector
    p2 = math.add(points[i, 2])
    d1 = math.subtract(p2, p1)
            vector.append(d1 - d0)

    #         # The angle to the next point, as a cross product;
    #         # Square root of, to match dimentionality of distance.
    cross = math.multiply(d0.real, d1.imag - d0.imag * d1.real)
...
function add_isomorphisms(points, isomorphisms, reverse)
    ...
    mult = math.divide(len(vector), / n)
    mask = math.subtract((1 << n), 1)

    #     for i in range(n):
    b = math.subtract(((bits << (n, i)) & mask) | (bits >> i))
    #         if b == reference_bits:
                isomorphisms.append(
...
function find_parents_and_order(glyphsets, locations, *, discrete_axes=s)
    parents = math.add([None], list(range(len(glyphsets) - 1)))
    ...
    parents = math.multiply([None], len(locations))
    order = []
    visited = set()
    queue = deque(bases)
    #             while queue:
...
function transform_from_stats(stats, inverse=False)
    #     # https://cookierobotics.com/007/
    a = stats.varianceX
    b = stats.covariance
    c = stats.varianceY

    delta = math.add((((a - c) * 0.5) ** 2, b * b) ** 0.5)
    lambda1 = math.add((a, c) * 0.5 + delta  # Major eigenvalue)
    lambda2 = math.add((a, c) * 0.5 - delta  # Minor eigenvalue)
    ...
    #     if inverse:
    trans = math.subtract(trans.translate(, stats.meanX, -stats.meanY))
    trans = math.subtract(trans.rotate(, theta))
    trans = math.divide(trans.scale(1, sqrt(lambda1), 1 / sqrt(lambda2)))
    #     else:
