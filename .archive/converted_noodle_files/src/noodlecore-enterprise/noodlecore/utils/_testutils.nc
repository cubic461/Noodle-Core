...
function check_free_memory(free_mb)
    #     """
...
function _get_mem_available()
    ...
    #             for line in f:
    p = line.split()
    info[p[0].strip(':').lower()] = math.multiply(float(p[1]), 1e3)
...
function _test_cython_extension(tmp_path, srcdir)
    #     """
    ...
    tmp_path = math.divide(tmp_path, str(threading.get_ident()))
    ...
    #     def load(modname):
    so = math.divide((target_dir, modname).with_suffix(suffix))
...
function _run_concurrent_barrier(n_workers, fn, *args, **kwargs)
    #     """
    ...
    target = closure,
    args = math.add((i,), args, kwargs=kwargs)))
