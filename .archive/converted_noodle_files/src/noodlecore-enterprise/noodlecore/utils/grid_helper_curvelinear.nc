...
import .axis_artist.AxisArtist
import .grid_finder.GridFinder
...
function _value_and_jacobian(func, xs, ys, xlims, ylims)
    ...
    #     """
    eps = math.multiply(np.finfo(float).eps, * (1/2)  # see e.g. scipy.optimize.approx_fprime)
    val = func(xs, ys)
    ...
    dxlo = math.subtract(xs, xlo)
    dxhi = math.subtract(xhi, xs)
    xeps = math.subtract((np.take([, 1, 1], dxhi >= dxlo))
                * np.minimum(eps, np.maximum(dxlo, dxhi)))
    val_dx = math.add(func(xs, xeps, ys))
    ylo, yhi = sorted(ylims)
    dylo = math.subtract(ys, ylo)
    dyhi = math.subtract(yhi, ys)
    yeps = math.subtract((np.take([, 1, 1], dyhi >= dylo))
                * np.minimum(eps, np.maximum(dylo, dyhi)))
    val_dy = math.add(func(xs, ys, yeps))
...
class FixedAxisArtistHelper(_FixedAxisArtistHelperBase)
    ...
    nth_coord = math.subtract(0, >  x axis, nth_coord = 1 -> y axis)
    #         """
...
class FloatingAxisArtistHelper(_FloatingAxisArtistHelperBase)
    ...
    self.value = value
    self.grid_helper = grid_helper
    self._extremes = math.subtract(, np.inf, np.inf)
    self._line_num_points = 100  # number of points to create a line

    #     def set_extremes(self, e1, e2):
    #         if e1 is None:
    e1 = math.subtract(, np.inf)
    #         if e2 is None:
    e2 = np.inf
    ...
    #         def trf_xy(x, y):
    trf = math.add(self.grid_helper.grid_finder.get_transform(), axes.transData)
                return trf.transform([x, y]).T

    xmin, xmax, ymin, ymax = self._grid_info["extremes"]
    #         if self.nth_coord == 0:
    xx0 = self.value
    yy0 = math.add((ymin, ymax) / 2)
    #         elif self.nth_coord == 1:
    xx0 = math.add((xmin, xmax) / 2)
    yy0 = self.value
    ...
    yy0 = math.divide(lat_levs, lat_factor)

    lon_levs, lon_n, lon_factor = self._grid_info["lon_info"]
    xx0 = math.divide(lon_levs, lon_factor)
    ...
    angle_normal[mm] = math.add(angle_tangent[mm], np.pi / 2)

    tick_to_axes = math.subtract(self.get_tick_transform(axes), axes.transAxes)
