...
class LeakTestCase(unittest.TestCase)
    ...
    #             return
            ...
            gc.collect()
    lost_i = math.subtract(_GetInterfaceCount(), ni)
    lost_g = math.subtract(_GetGatewayCount(), ng)
    #         if lost_i or lost_g:
    ...
    #         # failure, but one per loop
            gc.collect()
    lost = math.subtract((gtrc(), trc) // self.num_leak_iters)
    #         if lost < 0:
...
function find_test_fixture(basename, extra_dir=".")
