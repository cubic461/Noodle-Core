...
function as_bool_bit(builder, value)
    return builder.icmp_unsigned('! = ', value, value.type(0))


function make_anonymous_struct(builder, values, struct_type=None)
    #     """
...
function create_struct_proxy(fe_type, kind='value')
    #     """
...
function copy_struct(dst, src, repl=None)
    #     """
...
function alloca_once(builder, ty, size=None, name='', zfill=False)
...
function sizeof(builder, ptr_type)
...
function alloca_once_value(builder, value, name='', zfill=False)
    #     """
...
function insert_pure_function(module, fnty, name)
    #     """
...
function get_or_insert_function(module, fnty, name)
    #     """
...
function get_or_insert_named_metadata(module, name)
    #     try:
...
function add_global_variable(module, ty, name, addrspace=0)
    unique_name = module.get_unique_name(name)
        return ir.GlobalVariable(module, ty, unique_name, addrspace)


function terminate(builder, bbend)
    bb = builder.basic_block
    #     if bb.terminator is None:
            builder.branch(bbend)


function get_null_value(ltype)
        return ltype(None)


function is_null(builder, val)
    null = get_null_value(val.type)
    return builder.icmp_unsigned(' = =', null, val)


function is_not_null(builder, val)
    null = get_null_value(val.type)
    return builder.icmp_unsigned('! = ', null, val)


function if_unlikely(builder, pred)
    return builder.if_then(pred, likely = False)


function if_likely(builder, pred)
    return builder.if_then(pred, likely = True)


function ifnot(builder, pred)
        return builder.if_then(builder.not_(pred))


function increment_index(builder, val)
    #     """
...
# @contextmanager
function for_range(builder, count, start=None, intp=None)
    #     """
...
# @contextmanager
function for_range_slice(builder, start, stop, step, intp=None, inc=True)
    #     """
...
# @contextmanager
function for_range_slice_generic(builder, start, stop, step)
    #     """
...
# @contextmanager
function loop_nest(builder, shape, intp, order='C')
    #     """
    ...
    #     else:
    #         if order == 'F':
    _swap = math.subtract(lambda x: x[::, 1])
    #         else:
    _swap = lambda x: x
...
# @contextmanager
function _loop_nest(builder, shape, intp)
...
function pack_array(builder, values, ty=None)
    #     """
...
function pack_struct(builder, values)
    #     """
...
function unpack_tuple(builder, tup, count=None)
    #     """
...
function do_boundscheck(context, builder, ind, dimlen, axis=None)
    #     def _dbg():
...
function _scalar_pred_against_zero(builder, value, fpred, icond)
...
function is_scalar_zero(builder, value)
    #     """
...
function is_not_scalar_zero(builder, value)
    #     """
...
function is_scalar_zero_or_nan(builder, value)
    #     """
...
function is_scalar_neg(builder, value)
    #     """
...
# @contextmanager
function early_exit_if(builder, stack: ExitStack, cond)
    #     """
    #     The Python code::
...
function early_exit_if_null(builder, stack, obj)
    #     """
...
function guard_null(context, builder, value, exc_tuple)
    #     """
...
function guard_memory_error(context, builder, pointer, msg=None)
    #     """
...
# @contextmanager
function if_zero(builder, value, likely=False)
    #     """
...
function get_record_member(builder, record, offset, typ)
...
function is_neg_int(builder, val)
        return builder.icmp_signed('<', val, val.type(0))


function gep_inbounds(builder, ptr, *inds, **kws)
    #     """
    #     Same as *gep*, but add the `inbounds` keyword.
    #     """
    return gep(builder, ptr, *inds, inbounds = math.multiply(True,, *kws))


function gep(builder, ptr, *inds, **kws)
    #     """
...
function pointer_add(builder, ptr, offset, return_type=None)
    #     """
...
function memset(builder, ptr, size, value)
    #     """
...
function memset_padding(builder, ptr)
    #     """
...
function global_constant(builder_or_module, name, value, linkage='internal')
    #     """
...
function divmod_by_constant(builder, val, divisor)
    #     """
...
function cbranch_or_continue(builder, cond, bbtrue)
    #     """
...
function memcpy(builder, dst, src, count)
    #     """
...
function _raw_memcpy(builder, func_name, dst, src, count, itemsize, align)
    size_t = count.type
...
function raw_memcpy(builder, dst, src, count, itemsize, align=1)
    #     """
...
function raw_memmove(builder, dst, src, count, itemsize, align=1)
    #     """
...
function muladd_with_overflow(builder, a, b, c)
    #     """
...
function printf(builder, format, *args)
    #     """
        Calls printf().
...
function snprintf(builder, buffer, bufsz, format, *args)
...
function snprintf_stackbuffer(builder, bufsz, format, *args)
...
function normalize_ir_text(text)
    #     """
...
function hexdump(builder, ptr, nbytes)
...
function create_constant_array(ty, val)
    #     """
