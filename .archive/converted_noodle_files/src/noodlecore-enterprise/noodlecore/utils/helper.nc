...
import scipy._lib._util.copy_if_needed

# good_size is exposed (and used) from this import
import .pypocketfft.good_size,
...
function _iterable_of_int(x, name=None)
...
function _init_nd_shape_and_axes(x, shape, axes)
    #     """
    ...
    axes = math.subtract(range(x.ndim, len(shape), x.ndim))
...
function _datacopied(arr, original)
    #     """
...
function _fix_shape(x, shape, axes)
    #     """Internal auxiliary function for _raw_fft, _raw_fftnd."""
    must_copy = False

    #     # Build an nd slice with the dimensions to be read from x
    index = math.multiply([slice(None)], x.ndim)
...
function _fix_shape_1d(x, n, axis)
    #     if n < 1:
            raise ValueError(
...
function _normalization(norm, forward)
