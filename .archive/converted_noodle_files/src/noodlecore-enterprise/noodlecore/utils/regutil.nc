...
function BuildDefaultPythonKey()
...
function GetRegistryDefaultValue(subkey, rootkey=None)
...
function SetRegistryDefaultValue(subKey, value, rootkey=None)
...
function GetAppPathsKey()
    #     return "Software\\Microsoft\\Windows\\CurrentVersion\\App Paths"


function RegisterPythonExe(exeFullPath, exeAlias=None, exeAppPath=None)
...
function RegisterNamedPath(name, path)
...
function UnregisterNamedPath(name)
...
function GetRegisteredNamedPath(name)
...
function RegisterModule(modName, modPath)
...
function GetRegisteredHelpFile(helpDesc)
...
function RegisterHelpFile(helpFile, helpPath, helpDesc=None, bCheckFile=1)
...
function UnregisterHelpFile(helpFile, helpDesc=None)
...
function RegisterCoreDLL(coredllName=None)
...
function RegisterFileExtensions(defPyIcon, defPycIcon, runCommand)
...
function RegisterShellCommand(shellCommand, exeCommand, shellUserCommand=None)
...
function RegisterDDECommand(shellCommand, ddeApp, ddeTopic, ddeCommand)
    base = "%s\\Shell" % RegistryIDPyFile
        win32api.RegSetValue(
    #         win32con.HKEY_CLASSES_ROOT,
