...
import typer.core.MarkupMode,
import click.utils.make_str


import safety.constants.(
...
#     FeatureType,
# )
import safety.scan.constants.CONSOLE_HELP_THEME
import safety.models.SafetyCLI
...
function pass_safety_cli_obj(func)
    #     """
...
#     )
commands_table.add_column(width = math.subtract(console_width, column_width))
...
class CustomContext(click.Context)
    ...
    super().__init__(command, parent = math.multiply(parent,, *kwargs))
...
class SafetyCLILegacyGroup(click.Group)
    ...
    options["proxy_protocol"] = math.add(args[i, 1])
    #             elif arg in ["--proxy-port", "-pp"] and i + 1 < len(args):
    options["proxy_port"] = math.add(int(args[i, 1]))
    #             elif arg in ["--proxy-host", "-ph"] and i + 1 < len(args):
    options["proxy_host"] = math.add(args[i, 1])
    #             elif arg in ["--key"] and i + 1 < len(args):
    key = math.add(args[i, 1])
