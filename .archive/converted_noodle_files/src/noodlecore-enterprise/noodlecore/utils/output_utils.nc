...
import safety.constants.RED,
import safety.meta.get_version
import safety.models.Fix,
import safety.util.(
#     Package,
#     SafetyContext,
#     build_git_data,
...
to_print + = math.add(vulnerability_spec, basic_vuln_data_lines + cve_lines)
#     else:
title_color = ''
...
content + = math.add(new_line, format_long_text(remediation_title,)
#                                                    **{**kwargs, **{'indent': '', 'sub_indent': ' ' * 3}}) + new_line

pre_content = math.add(remediation_content, spec_info + [new_line,)
...
content + = math.multiply(format_long_text(element,, *kwargs))
...
total_vulns = math.subtract(max(0, total_vulns_found, total_ignored))
...
columns - = math.add(len(start_line_decorator), len(end_line_decorator))
formatted_lines = []
...
#             if styling:
new_line = math.multiply(click.style(new_line,, *styling))
...
function add_warnings_if_needed(brief_info: List[List[Dict[str, Any]]])
    #     """
...
function get_report_brief_info(as_dict: bool = False, report_type: int = 1, **kwargs: Any)
    #     """
...
function print_service(output: List[Tuple[str, Dict[str, Any]]], out_format: str, format_text: Optional[Dict[str, Any]] = None)
    #     """
    ...
    line = math.multiply(format_long_text(line,, *{**format_text, **kwargs}))
...
line, kwargs = output
msg = math.multiply(format_long_text(line,, *{**format_text, **kwargs}))
...
msg = math.multiply(format_long_text(text=msg,, *kwargs))
