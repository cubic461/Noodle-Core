...
class Lexer
    ...
    eof_token = Token(Token.EOF, "", eof_position)
    ...
    whitespace_token = Token(Token.ERROR, char, position)
    ...
    next_char = self.content[self.current_position + 1]
    #             if next_char == '/':
    ...
    comment_text = self.content[start_pos:self.current_position]
                    return Token(Token.ERROR, comment_text, position)
    ...
    position = self._create_position()
    char = self.content[self.current_position]
    error_token = Token(Token.ERROR, f"Unknown character: {char}", position)
    self.current_position + = 1
    #             return error_token
