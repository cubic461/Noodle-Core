...
class TaskScheduler
    ...
    base_priority = task.priority.value * 1000

    #         # Add dependency weight
    dependency_weight = len(task.dependencies) * self.policy.dependency_weight * 100

    #         # Add affinity weight
    affinity_weight = task.affinity_score * self.policy.affinity_weight * 100
    ...
    resource_weight + = scarcity * self.policy.resource_weight * 100
    ...
    waste = resource.available_units - req.amount
    #                     if waste < best_waste:
    best_waste = waste
...
class TaskIndex
    ...
    #             if (
    #                 resource.available_units + resource.allocated_units != resource.total_units
    #             ):
