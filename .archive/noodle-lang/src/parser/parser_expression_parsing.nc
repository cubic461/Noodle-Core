...
class ExpressionParser
    ...
    #             else:
    right = self.parse_binary_expression(current_precedence + 1)
    ...
    #         if token.type.value == Token.EOF.value:
    ...
    #             if token.type.value in [TokenType.SEMICOLON.value, TokenType.NEWLINE.value]:
                    self.eat(token.type)
    #                 break
    #             elif token.type.value == Token.EOF.value:
    #                 break
    #             else:
    self.token_index + = 1
    #                 if self.token_index < len(self.tokens):
    ...
    #                 try:
    #                     if expr.operator == '+':
    value = float(expr.left.value) + float(expr.right.value)
    #                     elif expr.operator == '-':
    value = float(expr.left.value) - float(expr.right.value)
    #                     elif expr.operator == '*':
    value = float(expr.left.value) * float(expr.right.value)
    ...
    parse_time = time.time() - self.parse_start_time
