...
class DistributedIndex
    ...
    load_factor = 1.0 - (node.load / 100.0  # Lower load = higher score)
    ...
    shard_end = min((i + 1 * shard_size, size))
    ...
    cost + = len(targets) * 0.1  # Base communication cost
...
class DistributedHashTable
    ...
    replica_index = (current_index + i + 1 % len(self.hash_ring))
