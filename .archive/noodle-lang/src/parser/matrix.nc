...
class Matrix
    ...
    new_shape = (self.rows - 1, self.cols - 1)
    ...
    mean_val = self.mean()
    variance = sum((self.data[i][j] - mean_val) * * 2
    ...
    start_idx = i * new_cols
    end_idx = start_idx + new_cols
...
#     # Stack them
return tensor.stack(tensors, axis = axis * , *kwargs)
