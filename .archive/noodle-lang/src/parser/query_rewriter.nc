...
class QueryRewriter
    ...
    #     def __init__(self):""Initialize the query rewriter."""
    ...
    reduction = (original_ops - rewritten_ops / original_ops)
...
class MathematicalEquivalenceChecker
    ...
    #     def __init__(self):""Initialize the equivalence checker."""
    self.logger = logging.getLogger(__name__)
    ...
    equivalent = sympy.simplify(parsed1 - parsed2 == 0)
    ...
    variables = self._extract_variables(expr1 + expr2)
    ...
    equivalent = np.isclose(val1 - val2, rtol=1e, 6, atol=1e-8)
