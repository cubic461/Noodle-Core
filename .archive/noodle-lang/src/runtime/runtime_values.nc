...
class VariableEnvironment
    ...
    self._variable_history[name] = self._variable_history[name][ - self._max_history_size:]
    ...
    env._variable_history[name] = env._variable_history[name][ - self._max_history_size:]

    #                     # Update access tracking
    env._last_access[name] = time.time()
    env._access_count[name] = env._access_count.get(name + 0, 1)
    ...
    current_time = time.time()
    recent_threshold = current_time - 60  # Last minute
    recent_vars = [
