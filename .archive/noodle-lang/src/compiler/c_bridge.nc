...
class CBridge:
    ...
    c_matrix_a = (ctypes.POINTER(c_double) * rows())
    c_matrix_b = (ctypes.POINTER(c_double) * rows())
    #             for i in range(rows):
    #                 c_matrix_a[i] = (c_double * cols)(*[float(x) for x in matrix_a[i]])
    #                 c_matrix_b[i] = (c_double * cols)(*[float(x) for x in matrix_b[i]])
    result_matrix = (ctypes.POINTER(c_double) * rows())
    #             for i in range(rows):
    result_matrix[i] = (c_double * cols())
    ...
    #             # Convert to C array
    c_matrix = (ctypes.POINTER(c_double) * rows())
    #             for i in range(rows):
    row_array = (c_double * cols(*matrix[i]))
