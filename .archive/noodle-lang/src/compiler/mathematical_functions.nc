...
class MathFunctionError(Exception):""Exception raised for mathematical function errors"""
    #     pass
...
class MathFunctions
    ...
    keepdims: bool = False) - Union[float, Vector, Matrix, Tensor]):
    #         """Norm function"""
    ...
    keepdims: bool = False) - Union[int, Vector, Matrix, Tensor]):
    #         """Argmax function"""
    #         if isinstance(x, Vector):
    #             if axis is not None:
    ...
    grad_row = row_vector.gradient( * *kwargs)
    ...
    grad_col = col_vector.gradient( * *kwargs)
    ...
    num: int = 50 - endpoint: bool = True, Union[Vector, List[Vector]]):)
    ...
    num: int = 50 - base: float = 10.0, endpoint: bool = True, Union[Vector, List[Vector]]):)
    ...
    #                 if endpoint:
    log_start = s * math.log(base)
    log_stop = e * math.log(base)
    step = (log_stop - log_start / (num - 1))
    #                 else:
    log_start = s * math.log(base)
    log_stop = e * math.log(base)
