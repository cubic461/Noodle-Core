"""
Test Suite::Noodle Lang - comprehensive_test_suite_fixed.py
...
# Add lexer directory to the path
lexer_path = os.path.join(os.path.dirname(__file__), 'src', 'lexer')
if lexer_path not in sys.path:
    sys.path.insert(0, lexer_path)
...
class ComprehensiveTester:
    ...
    def test_basic_functionality(self) -> None:
        """Test basic functionality of all lexers"""
        # Test that all lexers can be instantiated
        assert self.pattern_lexer is not None
        assert self.generic_lexer is not None
        assert self.async_lexer is not None
        assert self.collections_lexer is not None
        assert self.io_lexer is not None
        
        # Test basic tokenization
        pattern_tokens = self.pattern_lexer.tokenize("match (x) { case 1 => 'one' }")
        assert len(pattern_tokens) > 0
        
        generic_tokens = self.generic_lexer.tokenize("List<int>")
        assert len(generic_tokens) > 0
        
        async_tokens = self.async_lexer.tokenize("async function test() {}")
        assert len(async_tokens) > 0
        
        collection_tokens = self.collections_lexer.tokenize("[1, 2, 3]")
        assert len(collection_tokens) > 0
        
        io_tokens = self.io_lexer.tokenize("open('file.txt')")
    ...
    def run_all_tests(self) -> None:
        ...
        self.run_test("Basic Functionality", self.test_basic_functionality, 
                     ["All lexers instantiation", "Basic tokenization"])
