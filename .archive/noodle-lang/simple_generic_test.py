"""
Test Suite::Noodle Lang - simple_generic_test.py
...
class GenericTypeTester:
    ...
    def test_basic_type_parameter_tokenization(self) -> None:
        ...
        assert tokens[0].type == GenericTypeLexer.LT, "First token should be LT"
        assert tokens[1].type == GenericTypeLexer.IDENTIFIER, "Second token should be IDENTIFIER"
        assert tokens[1].value == "T", "Identifier should be 'T'"
        assert tokens[2].type == GenericTypeLexer.GT, "Third token should be GT"
    ...
    def test_type_constraints(self) -> None:
        ...
        assert tokens[0].type == GenericTypeLexer.LT
        assert tokens[1].value == "T"
        assert tokens[2].type == GenericTypeLexer.COLON
        assert tokens[3].value == "Numeric"
        assert tokens[4].type == GenericTypeLexer.GT
